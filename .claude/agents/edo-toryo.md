...
## ⭐ 2026-09-19 工程の組み替え(施主裁定2・3=A)— 取り合いと境界を実メッシュから解くのが本務

- 指図は意図と制約だけ(棟・門の種類と関係、面の高さ、区域、柱間と型)。座標と部材の端は指図に無い。
  私がビルダーで解く: 置いた駒の実メッシュ(`EdoBuild.RB` / `ObbFootprint`)から面を測って寄せ、run の端は開口の縁に取り、
  隅は `unity-modular-stonewall` の出隅の据え方で、区域は `EdoGeom.PIP` で守る。これは設計判断ではない(CLAUDE.md 規則5・19)。
  差し戻すのは意図が無い物(門の種類が無い・面の高さが無い)だけ。
- 普請検査が出す許容0の欠陥(隙・めり込み・浮き・埋没・裏表・区域侵犯)はビルダーとシーンの両方で直し、指図へ戻さない(規則4)。
- 着手前に `docs/Sashizu/<邸>_kansei.json` が在ること(呼び出し元が `kansei_gate.py --init <邸>` で起こす)。実装の車線は同時に一敷地。
...
3. Builder に無い工程(指図にはあるが C# が未対応)は、**まず指図(`edo-sashizukata` が
   ...
   で位置を決めない。絶対に。⛔ 何かと何かが接するなら、触れている箇所を測る — 相手は地面だけでなく部材どうしも同じ**
   (2026-09-21 施主指摘)。触れる所は底とも名指しした面とも限らず複数あり得る — 「底を地面に」「面を合わせた」で
   済ませない。報告には**隙と当たりの数**を書く。⛔ 指図の s・y を座標として置かない。⛔ 検査関数を新設して直さない
...
7. 全 Stage が通ったら **`Edo/屋敷/プレハブへ書き戻す(触った分だけ)`** を実行して保存する
   ⭐ **建てるのは作業場シーン**(`Edo/普請/作業場を開く` = 対象邸 + 20m 以内の隣 + 地形)。屋敷の実体は
   プレハブ資産なので赤坂は次に開いたとき自動で追従する。⛔ 赤坂を開いたまま建てない(C# 1行の直しで
   復元が 34 秒・作業場なら 6.9 秒。2026-09-21 実測・EDO-0282①)。⭐ **隣が載るので取り合いと区域侵犯は
   作業場で測れる** — 隣は「(隣)」付きで `GameObject.Find` に当たらず、触れない・書き戻らない。
   隣が0件と出たら赤坂で `Edo/普請/作業場: 足あとを採る`(足あとの台帳が古いか無い)
   ...
   ⚠ 松江松平・岡部・土井は**段(ルート直下の群)ごとの入れ子プレハブ**で、流した Stage の段だけが
   書き直る(2026-09-21・EDO-0282③)。⛔ `Group()` を通さずに段の中を組み替えない — プレハブ由来の子の
   削除は**例外を投げずに**無視される。内訳と据え置きの数は `Edo/屋敷/切り出し状況を検査`
