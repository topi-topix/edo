...
if os.path.exists(CLI):
    ...
    if os.path.exists(rcli):
        ...
        if r.stdout.strip():
            ...
            print("  ⚠ **【移行期間】2026-09-01 ユーザー裁定(案B)。**関門は新設で全邸が赤で"
                  "立ち上がったため、**検分を通すまで従来どおり作業を続けてよい**。"
                  "⛔ **ただし新たにユーザーへ見せる前には必ず通す**"
                  "(見せる=裁定を仰ぐ・レンダを出す・指図の Artifact を案内する)。\n"
                  "  ⛔ **「記録が無い」は「通していない」ではない** — 関門より前の検分は"
                  "記録されていないだけ。⛔ それでも遡って pass を書かない(過去の検分は"
