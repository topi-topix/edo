...
- **建て直しの輪は作業場シーンで回す。**(2026-09-21・EDO-0282①)赤坂を開いたままだと、C# を1行
  直すたびに Unity が 83 プレハブを退避して復元する。実測(1起動ぶん・`Tools/Unity/reload_cost.py`):
  **赤坂 15回 531秒(中央値 34.3s)/ 作業場 2回 11秒(中央値 6.9s)** — その起動だけで 428 秒が溶けた。
  ⭐ **隣も載る。** `Edo/普請/作業場を開く`(対象邸 + 20m 以内の隣 + 地形)。取り合いと区域侵犯は隣が
  居ないと測れないので、足あとの台帳 `UserData/Koba/ashiato.json` から機械で選ぶ。段へ割ってある邸は
  **囲いの段だけ**載る(松江松平 42MB → 1.8MB)。隣は名前に「(隣)」が付き、ビルダーの
  `GameObject.Find(ルート名)` に当たらない = 触れない・書き戻らない。台帳が古い/無いときは
  赤坂で `Edo/普請/作業場: 足あとを採る`。屋敷の実体はプレハブ資産なので保存すれば赤坂は自動で追従する。
  ⛔ **検証レンダと全域の見え方は作業場では見えない** — 最後に `Edo/普請/赤坂へ戻す` で検める。
  ⚠ 赤坂を開いたままコンパイルすると、その場で ⚠ が出る(`EdoKobaNudge`)。手仕舞いの
  `release --resources unity` は、その起動で**どのシーンに何秒溶かしたか**を突きつける。
