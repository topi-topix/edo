...
// 【地形】三べ坂=緑線に沿う実在の坂(南h15→北h29)。
//   ⚠【2026-08-13 方針変更(ユーザー指示+史料調査)】「造成ゼロ・地形追従」を改め、渡辺邸は拝領時造成を
//   再現して段切り(雛壇 T4=16/T3=20/T2=24+台地は現状)した。典拠: 菊地真2003「長府藩毛利家屋敷地の
//   事例に見る江戸城下町の形成と土地改変」歴史地理学45-3(発掘67遺跡集成。「大名屋敷建設にともない
//   大規模な切土・盛土を行い敷地を整地する類の開発が屋敷単位で進行」「盛土は高さ1-4mが多い」実例:
//   紀伊家紀尾井町5.0m・汐留1.5m・長府毛利3.0m)/富山藩邸発掘(台地縁の傾斜地=「段切り」で複数平坦面)。
//   バックアップ=scratchpad/watanabe_dankiri_backup_772_1028_81x66.bin。段間は土手(法面)。石垣化は将来。
// 【2026-08-13 雛壇長屋の撤去(ユーザー差し戻し)】西斜面を2.5m刻みの雛壇4段に刻んで家臣長屋34棟を
//   並べたが(旧commit 8efb7c6)、これは「空いている土地に長屋をぽんぽん建てて建蔽率を上げる」という
//   禁止事項に当たるとして差し戻された。KND* を SetActive(false)、雛壇の造成も自然地形へ復元し、
//   斜面は庭(松・竹・刈込26本を補植)に戻した。建蔽率 34.2%→24.2%。
//   ⚠ 整地の目的は「建物が載る大きな平場を作ること」。斜面に細い段を刻んで長屋で埋めるのは不可。
//   原則は unity-buke-yashiki スキル §B-1/§B-3。
