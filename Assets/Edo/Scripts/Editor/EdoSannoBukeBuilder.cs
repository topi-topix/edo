...
//     **表門=北向き**(下の訂正 2026-08-31)。南東辺は溜池水際に直接面する。跡地=山王パークタワー一帯。
...
//   ・**内藤も同型だった(2026-08-31 解決)**: 旧注記「表門=北西向き」は誤りで、**北向きが正**。
//     実装(L66・Stage6「北辺東寄り x≈-106・北向き」)が正しい。3邸ともヘッダだけが旧説だった。
//   ・⭐ **同一図の中で文字の組み方が門の向きを示す**(丹羽の考証方が対照実験で確認):
//     **丹羽・京極=横一行・頭が右 → 東向き** / **内藤=縦一列・頭が上 → 北向き**。
//     縦横の割れは作図の癖ではなく門の向きの記号として機能している。
