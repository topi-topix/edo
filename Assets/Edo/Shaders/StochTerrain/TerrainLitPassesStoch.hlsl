...
// ---- Stochastic (hex) tiling: タイル反復を解消。1枚のテクスチャをセルごとに
//      ランダムオフセットしてサンプリングし、三角形重心で3枚をブレンドする(Heitz-Neyret簡易版)。----
float2 EdoHex_Hash(float2 p){ return frac(sin(mul(p, float2x2(127.1,311.7,269.5,183.3))) * 43758.5453); }
void EdoHex_Triangle(float2 uv, out float w1, out float w2, out float w3, out float2 v1, out float2 v2, out float2 v3)
{
    uv *= 3.464101615;                                    // 2*sqrt(3)
    const float2x2 toSkew = float2x2(1.0, 0.0, -0.5773502692, 1.1547005384);
    float2 s = mul(toSkew, uv);
    float2 baseId = floor(s);
    float3 f = float3(frac(s), 0.0); f.z = 1.0 - f.x - f.y;
    if (f.z > 0.0) { w1 = f.z;  w2 = f.y;      w3 = f.x;      v1 = baseId;               v2 = baseId + float2(0,1); v3 = baseId + float2(1,0); }
    else           { w1 = -f.z; w2 = 1.0-f.y;  w3 = 1.0-f.x;  v1 = baseId + float2(1,1);  v2 = baseId + float2(1,0); v3 = baseId + float2(0,1); }
}
half4 EdoHexSample(TEXTURE2D_PARAM(tex, smp), float2 uv)
{
    float w1, w2, w3; float2 v1, v2, v3;
    EdoHex_Triangle(uv, w1, w2, w3, v1, v2, v3);
    float2 dx = ddx(uv), dy = ddy(uv);                    // ミップ用の微分(オフセットは無視)
    half4 c1 = SAMPLE_TEXTURE2D_GRAD(tex, smp, uv + EdoHex_Hash(v1), dx, dy);
    half4 c2 = SAMPLE_TEXTURE2D_GRAD(tex, smp, uv + EdoHex_Hash(v2), dx, dy);
    half4 c3 = SAMPLE_TEXTURE2D_GRAD(tex, smp, uv + EdoHex_Hash(v3), dx, dy);
...
void NormalMapMix(float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, inout half3 mixedNormal)
{
        ...
        half3 nrm = half(0.0);
        nrm += splatControl.r * UnpackNormalScale(EdoHexSample(TEXTURE2D_ARGS(_Normal0, sampler_Normal0), uvSplat01.xy), _NormalScale0);
        nrm += splatControl.g * UnpackNormalScale(EdoHexSample(TEXTURE2D_ARGS(_Normal1, sampler_Normal0), uvSplat01.zw), _NormalScale1);
        nrm += splatControl.b * UnpackNormalScale(EdoHexSample(TEXTURE2D_ARGS(_Normal2, sampler_Normal0), uvSplat23.xy), _NormalScale2);
        nrm += splatControl.a * UnpackNormalScale(EdoHexSample(TEXTURE2D_ARGS(_Normal3, sampler_Normal0), uvSplat23.zw), _NormalScale3);
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight, out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal)
{
    ...
    diffAlbedo[0] = EdoHexSample(TEXTURE2D_ARGS(_Splat0, sampler_Splat0), uvSplat01.xy);
    diffAlbedo[1] = EdoHexSample(TEXTURE2D_ARGS(_Splat1, sampler_Splat0), uvSplat01.zw);
    diffAlbedo[2] = EdoHexSample(TEXTURE2D_ARGS(_Splat2, sampler_Splat0), uvSplat23.xy);
    diffAlbedo[3] = EdoHexSample(TEXTURE2D_ARGS(_Splat3, sampler_Splat0), uvSplat23.zw);
