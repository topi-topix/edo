...
4. **建った敷地の直しは欄の上書き。図は開かない。** 経緯は `git log`。数値は `docs/Sashizu/*.json` にのみ置く。
   建てて出た欠陥は二分類: **許容0**(隙・めり込み・浮き・埋没・裏表・区域侵犯)はビルダーとシーンで直す /
...
21. **ビルダーは2本。邸ビルダーに置き方を書かない。**(2026-09-20 施主裁定「不要なビルダーは削除して、新しい仕組みに
    早く置き換えろ」)建てるのは **類型ビルダー**(`EdoTypologyBuilder`・類型表を読む)と **指図ビルダー**(指図 json を読む)
    の2本だけ。部材を置く・測る・突き付ける・接地箇所を測って据えるのは **`EdoBuild` の関数だけ**で行い、邸や街区ごとの
    ビルダーに置き方を書かない。⛔ 指摘を受けた箇所だけ直す「局所修正」を禁じる — 直しは `EdoBuild` へ入れ、
    報告に「この型は他のどの部材・どの邸に当たるか」を一行添える。由来: 「接するところを測って建てる」を
    8/29・9/6・9/13・9/19 と四度文章に書いて四度漏れた(`docs/lessons.md`)。文章の規則は読む人が当てはめない限り
    効かない。効くのは共通の層の関数だけ。廃止の順は `docs/typology-builder.md` と掲示板 EDO-0289。
    ⛔ **部材の基準点(ピボット・原点・bounds の中心)で位置を決めない。絶対に。**接する所は**接地箇所を測って**据える。
    接地は「底」とは限らない(斜面では上手側の頂点が先に着く・据え面のある石はその縁・木は根張りの端)し、
    複数あり得る。「底を地面に」「実物の底」で済ませない — `EdoBuild.Contact`(接地箇所と隙間と接地の数)を測り、
    `SeatOnGround`/`SeatOnCrest`/`Abut` はそれで据える(2026-09-20 施主指摘)。
