...
4. **建った敷地の直しは欄の上書き。図は開かない。** 経緯は `git log`。数値は `docs/Sashizu/*.json` にのみ置く。
   建てて出た欠陥は二分類: **許容0**(隙・めり込み・浮き・埋没・裏表・区域侵犯 — ただし**区域侵犯は壁体で数え、
   軒の張り出しは許容する**。2026-09-21 施主裁定A「長屋や町家の軒が路上へ出るのは江戸の街並みとして自然」。
   軒の量は記録として別に刷る)はビルダーとシーンで直す /
...
21. **ビルダーは2本。邸ビルダーに置き方を書かない。**(2026-09-20 施主裁定「不要なビルダーは削除して、新しい仕組みに
    ...
    の2本だけ。部材を置く・測る・突き付ける・接触箇所を測って据えるのは **`EdoBuild` の関数だけ**で行い、邸や街区ごとの
    ...
    **何かと何かが接するなら、触れている箇所(接触箇所)を測って決める。**相手は地面だけではない —
    部材どうし・屋根と塀・隅と塀・石と土台も同じ(2026-09-21 施主指摘)。触れる所は「底」とも「名指しした面」とも
    限らない(斜面では上手側の頂点・据え面のある石はその縁・木は根張りの端・庇の裏や基壇の縁)し、複数あり得る。
    「底を地面に」「面どうしを合わせた」で済ませない — `EdoBuild.Contact` を測る
    (相手が地面: `Contact(駒)` / 相手が駒: `Contact(駒, 相手, 向き)` — 返るのは隙とその箇所と当たりの筋数)。
    寄せるのは `SeatOnGround`/`SeatOnCrest`/`Abut(駒, 相手, 向き, 隙)`。報告には**隙と当たりの数**を書く。
