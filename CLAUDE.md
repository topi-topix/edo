...
16. **一通に一種別。種別を見出しに立て、すべての項目に番号と題を振る。**
    ...
    **③どこ** を添える。⭐ 判定法は「**その1行だけ見せて相手が中身を言い当てられるか**」
    ...
    → 正典 **`docs/reporting-protocol.md`**。**本文脈・全エージェント・掲示板・セッション間に例外なく効く。**
...
17. **意匠を決める役と、書き起こす役を混ぜない。** 庭・石垣・建具のように**専門の検分役が
   ...
   ⛔ 本文脈が素人考えで組んだ案を「決まった大方針」として指図方へ渡さない
...
② 考証 + 指図  切絵図・史料から本文脈がユーザーと大方針を決める → edo-sashizukata が
...
| 作業 | エージェント |
|---|---|
| 大方針(本文脈でユーザーと決定済み)を**実装できる数値へ書き起こす** | **`edo-sashizukata`**(指図方・書き込み可) |
...
返事の要らないものと混ぜない。**本文脈・エージェント・掲示板・セッション間すべてに効く。**
