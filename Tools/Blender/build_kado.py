...
PARTS = {
    "ishigaki": dict(kit="JC",  src="Exterior/Castle Wall.fbx",
                     out="Ishigaki_Kado", axis="y", thk="min", pitch=2.0),
    ...
    "tsuijibei": dict(kit="OBJ", src=os.path.join(ROOT, "Assets/Edo/Models/Tsuijibei/Tsuijibei2m.fbx"),
                     out="Tsuijibei_Kado", axis="x", thk="center", pitch=2.0),
    ...
    "dobei":    dict(kit="PAIR", src=os.path.join(ROOT, "Assets/edogoyomi/es_dobei/s_hei_center.obj"),
                     out="Dobei_Kado", axis="x", thk="keep", pitch=1.645),
    "nagaya":   dict(kit="OBJ", src=os.path.join(ROOT, "Assets/edogoyomi/es_knagaya/knagaya01c.obj"),
                     out="Nagaya_Kado", axis="x", thk="keep", pitch=4.296),
}
...
def load(part):
    """素のモジュールを1つのメッシュとして読み、正規化する前の状態で返す。"""
    p = PARTS[part]
    if p["kit"] == "PAIR":
        return load_pair(p["src"])
    if p["kit"] == "OBJ":
        before = set(bpy.data.objects)
        path = p["src"]
        if path.lower().endswith(".obj"):
        ...
        meshes = V.imp(p["src"])
    return V.join(meshes, "src")


def normalize(o, part):
    ...
    # 走り: y ∈ [0, run] にそろえる(折れ点 y=0、腕は +Y 側)
    run = bb[1][1] - bb[1][0]
    dy = -bb[1][0]
...
def build(part, deg, arms=None):
    ...
    o = V.join([armA, armB], "%s_%02d%s" % (p["out"], round(abs(deg)), "" if deg > 0 else "M"))
