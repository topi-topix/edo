...
・勝手口 = 南面の**東の柱間**(u −28.75〜−27.75・板戸を半ば引いた姿)【U 庭方 — 賄の道の芯 u −28.0】。東西の面の 2・4 番目の柱間に連子窓(裏に明かり障子)【U】。
...
def build(name):
    ...
    # ⭐ 勝手口 = 南面の**東の柱間**(渡廊下の口と同じ柱間)。賄の道が芯 u −28.0 で北へ上がってくるので、
    #   中の柱間だと道が観音堂の前で曲がり離れが 0.15 間に詰まる(庭方の判断 2026-09-15)
    wall_run(M, uv, "u", -hv, us, floor, top, {gi: "itado"}, -1)
...
def shots(o, info):
    ...
    one((13.0, 15.0, 8.5), (0.0, 0.0, 2.6), "sw_oblique")                    # 南西(西面)
    one((-9.0, 12.0, 3.2), (-1.8, hv, 1.5), "se_katteguchi", res=(1400, 1100))  # 南東(勝手口 = 南面の東の柱間)
