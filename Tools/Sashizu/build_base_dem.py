...
SLICES = [
    ...
    dict(
        name="niwa_sakyo_dem.json", x0=-396, z0=660, step=2, nx=146, nz=123, nd=2, style="compact",
        parcels=["sannobuke_niwa"],
        doc="丹羽左京大夫上屋敷(陸奥二本松藩)まわりの造成前の地形【確度P】。世界座標 2m 刻み(x0,z0 から)。"
            "h[iz][ix]=標高m。指図の現況図(段彩+等高線)・切盛図・断面がこれを読む。"
            "⚠ **現代**の地面で、跡地(衆議院議員会館〜首相官邸)の近代の掘削と盛土を含む — "
            "江戸期の復元地盤は niwa_sakyo_edo_recon.json / niwa_sakyo_edo_world.json。",
        # ⭕ 2026-09-01(P-11 解決): 正本が b32e2bc で東と南へ広がったので、当邸だけ別経路だった
        #   `build_niwa_sakyo_dem.py`(参照ハイトマップから直に切り出す)を**捨てて正本へ相乗り**した。
    ),
    dict(
        name="naito_kii_dem.json", x0=-414, z0=478, step=2, nx=202, nz=124, nd=2, style="compact",
        parcels=["sannobuke_naito"],
        doc="内藤紀伊守中屋敷(越後村上藩)まわりの造成前の地形【確度P】。世界座標 2m 刻み(x0,z0 から)。"
            "h[iz][ix]=標高m。指図の現況図(段彩+等高線)・切盛図・断面がこれを読む。"
            "⚠ **現代**の地面で、跡地(山王パークタワー・首都高〜溜池交差点)の近代造成を含む。"
            "溜池は埋め立てられているので、南辺の汀線は現地形からは出ない。",
        # ⚠ 東 x=-12・南 z=478 は正本 CANON_SPEC の縁そのもの。区画+余白40m がぎりぎり入るだけで、
        #   これ以上は広げられない。広げるなら CANON_SPEC の再拡張(ref_height.npy の範囲内かの確認)から。
    ),
]
