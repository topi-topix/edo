    ...
    for m in d["munes"]:
        ...
        for r in m["rooms"]:
            ...
            if kz:
                ...
                # ⭐ **据える壁面を描く**(2026-09-08・第30次)。⛔ 第29次まで
...
def section_svg(d, sec):
    ...
    #   (2026-09-07・考証方 高・第26次: 断面内の凡例だけ旧文言のまま残っていた)。
    # ⛔⛔ **2026-09-08(第28次)に断りを復活させた。**この断面は屋根を帯割りではなく
...
def gate_svg(d):
    ...
    ⭐ **並びは `plan.sPos` が決める**(2026-09-08・第28次)。⛔ 描く順を関数に埋めない —
...
def runs_table(d):
    ...
    return ('<div class="tw"><table><thead><tr><th>run</th><th>辺</th><th>走り s</th><th>長さ</th>'
            ...
            "<br>⭐ <b>注記の欄は 2026-09-08(第30次)に足した</b> — "
...
def plane_check(d):
    ...
    # ⭐ **2026-09-08(第30次)に新設して同じ巡で配線した**(規則19【庭方 設計 D-5】)。
    ...
    # ⭐ **2026-09-10(第33次)に新設して同じ巡で配線した**(規則19)。
    ...
    # ⭐ **2026-09-14(第39次)に新設して同じ巡で配線した**(規則19・検図)
...
def section_roof_status(d, sec):
    ...
    **手で書いた棟名リストは持たない** — section_crossings と同じく棟の u/v から算出する。
    ...
    ⛔⛔ **2026-09-08(第28次): どちらも『屋根は概略・突き合わせ対象外』。**
      ...
      ⚠ 2026-09-07(第23次)に「帯割りが確定した棟から順に断りを外す」としたのは、
...
def section_roof_disclaimer_check(d):
    ...
    ⛔ 断りを外してよいのは `section_svg` が帯割り(`roof.bands`)から屋根を描くようになってから。
      ...
      『突き合わせの対象』として12面に出ていた(2026-09-08・第28次で戻した)。"""
...
VOCAB = [
    ...
    # ⭐ 2026-09-08(第28次): 表向の列の大棟を v へ回したので「流れ谷」を足した。
...
def gate_parts_table(d):
    ...
    # ⭐ **位置は `sPos` から導く**(2026-09-08・第28次)。⛔ 並び順(番所→袖塀→門柱)を
    ...
    return ("<h3>門構えの部材位置</h3><div class='tw'><table><thead><tr><th>部材</th><th>芯の世界座標 (x,z)</th>"
            ...
            "⭐ <b>2026-09-08(第28次)に並びを入れ替えた</b> — <b>番所は門柱へ直付け</b>"
...
def _opening_span(d, key):
    ...
    if key == "omote":
        sp = g["plan"]["sPos"]
        # ⭐ **門構えの両端は並び順に依らず `sPos` の最小・最大から採る**(2026-09-08・第28次)。
...
def _gate_kazari_note_checks(d):
    """第29次で足した3本+第30次で足した3本の束(素の設計と感度試験が**同じ束**を見る
...
def joints_face_table(d):
    ...
    return ("<h3>取り合い表 — 接する面で決める</h3><div class='tw'><table><thead><tr>"
            ...
            "⛔ 第29次の検図方の所見『取り合い表には既に出ている』は誤りで、"
...
def joints_check(d):
    ...
    # ⭐ **門構えの中の継ぎ目にも取り合いが要る**(2026-09-08・第28次)。
...
def route_keepout(d, route_name, role, ovr=None):
    """**その役に効く園路の退避[間]。**⛔ 一つの定数ではない(2026-09-08・第30次【庭方 D-4】)。
...
def free_fn(d):
    ...
    # ⭐ **園路ごとの半幅**(2026-09-08・第30次【庭方 設計・D-4】)。`keepoutByRole` の値が
    ...
    def _ovr_value(val, nm, mg):
        ...
        (2026-09-08・第30次)。⛔ 一定値ではない — 幅の違う園路に同じ数を当てない。
    ...
    def hit(u, v, clr, role=None, skip=None, pondClr=None, crownR=0.0, ovr=None, capR=None):
        ...
        for kind, g, mg, nm, bb, kk, spr in shapes:
            ...
            # ⭐ **物を名指しした上書き**(`keepoutByLayer.byName`・2026-09-08・第30次)。
            ...
            if ovrN:
                for _k9, _v9 in ovrN.items():
                    if _k9 in nm:
                        ...
                        break
            if kk in exempt:
                # ⭐ **点景の群の外接半径 `spread` はここでも足す**(2026-09-08・第30次)。
...
# ---------------------------------------------------------------- 庭の散布
_GSC = {}
# ⭐ `_GSC` と**同じ並び**で「その株がどの塊のものか」を持つ(2026-09-08・第30次)。
...
def scatter_gardens(d):
    ...
    for pl in d.get("planting", []):
        ...
        # ⭐ **詰め率は層・塊ごとに外せる**(2026-09-08・第30次【庭方 D-2/D-3】)。
        ...
        # ⭐ **塊ごとの上書き**(2026-09-08・第30次【庭方 D-1〜D-3】)。同じ層の中でも、
        ...
        def put(u, v, i):
            ...
            # ⭐ **どの塊の株か**を残す(2026-09-08・第30次)。⛔ 層でまとめて測ると、
        ...
        if pl.get("at"):                           # **一本ずつ位置が決まっている木**
            ...
            pool = {}
            # ⭐ **塊ごとに種を混ぜる**(2026-09-10・第33次【庭方 裁定A-2】)。
            ...
            for gi, gs in enumerate(pl["groups"]):
                ...
                for api9, cnt9 in gs["mix"].items():
                    ...
                    _taken |= set(av)
                    per.append(av)
                # ⭐ **交互に並べる**(2026-09-10・第33次)。⛔ 種ごとに固めて並べて
            ...
            for gi, gs in enumerate(pl["groups"]):
                g = int(gs["n"])
                # ⭐ **塊ごとの上書きをここで効かせる**(2026-09-08・第30次)。
            ...
            if ok and _pip_world(w, P):
                ...
                if ne_edge in toes:
                    ...
                    if None not in (yc, yt, y):
                        ...
                        # ⭐ **境を直線に切らない低周波のゆらぎ**(2026-09-10・第33次
...
_SPL = {}
_THIN = []          # ⭐ 突出木の周りを透かした記録 [(突出木の層, 替えた木の u, v, 元の部材)](2026-09-14 第39次)
...
def scatter_slope(d, dem):
    ...
    for lay in d.get("slopePlanting", []):
        ...
        for _z8 in d.get("slopePlantingZones", []):
            ...
            _zoff += len(_zp)
        # ⭐ **突出木を先頭へ寄せる**(2026-09-10・第33次【庭方 裁定A-3】)。
        ...
        if lay.get("placement") == "crestLine":
            ...
            if lay.get("placement") == "maxGap":
                # ⭐ **突出木は法肩の弧長で割り付ける**(2026-09-10・第33次【庭方 裁定A-3】)。
            ...
            for lay2 in d.get("slopePlanting", []):
                ...
                for i, (u, v, pt) in enumerate(lst):
                    if math.hypot(u - nu, v - nv) < 0.5:
                        ...
                        lst[i] = (u, v, pt2)
    # ⭐ **突出木の周りを透かす**(2026-09-14 第39次【庭方】`emergent.thin`)。
...
def planting_clearance_check(d, dem, extra=None):
    ...
    for pl in d.get("planting", []):
        ...
        z = zones.get(pl["zone"])
        role = pl.get("role", "")
        # ⭐ **株ごとに『その株が属する塊の上書き』で測り直す**(2026-09-08・第30次)。
...
def _box_blockers(d, pl, where):
    ...
    for i in range(11):
        for j in range(11):
            ...
            tot += 1
            # ⭐ 塊の上書きを重ねる(2026-09-08・第30次)— ⛔ 層の値だけで測ると
            #   散布器と別の集合を見る
...
def plant_height_check(d):
    ...
    for pl in d.get("planting", []):
        ...
        for pt in pl.get("parts", []):
            ...
            if not g:
                continue
            # ⭐ **最も大きく育つ個体で測る**(2026-09-10・第33次)。⛔ 従前は `scale` だけを
...
def group_pack_check(d, step=0.10, trials=400):
    ...
    for pl in d.get("planting", []):
        ...
        for gs in pl.get("groups", []):
            ...
            # ⭐ **塊ごとの上書きを散布器と同じ順で解く**(2026-09-08・第30次)。
...
DESIGN_WATCH = [
    ...
    # ⭐ **2026-09-10(第33次)に見張りへ入れた**(規則19)— 庭方 裁定A-2/A-3・普請奉行 B-1/B-3
    ...
    for pl in d.get("planting", []):
        ...
        for (u, v, pt) in got:
            ...
            if pt.get("scale") is not None:
                row["scale"] = pt["scale"]
            # ⭐ **部材ごとの大きさの揺らぎも書き出す**(2026-09-10・第33次【庭方 裁定A-2】)。
...
def _garden_checks(e, dem):
    ...
    return (planting_stock_check(e) + planting_clearance_check(e, dem)
            ...
            # ⭐ **2026-09-10(第33次)に感度試験の対象へ入れた**(規則19・庭方 裁定A-2/A-3)
...
def planting_sensitivity(d, dem):
    ...
    # ⭐ **2026-09-14(第39次)— 表裏・内法・社の正面・植栽の箱**(規則19・検図/庭方)
...
def slope_section_svg(d, dem, vcut, half=6.0):
    ...
    ridge = 0.0
    if mune:
        # ⭐ **大棟は帯割りから採る**(2026-09-08・第28次)。⛔ 棟の外形の梁間から出さない —
...
def neishi_show(d):
    ...
    ⛔ **指図は `show` を literal で持たない**(2026-09-10・第33次)。
...
def note_cell(it):
    """その項の `_` 接頭の注記を**ひと欄にまとめて刷る**(2026-09-08・第30次)。
...
def zashiki_kazari_table(d):
    ...
    return ("<h3>座敷飾 — 上段の間の設え</h3><div class='tw'><table><thead><tr>"
            ...
            " — 現存御殿は城郭附属で藩邸ではない。"
            "<br>⭕ <b>据える壁面は 2026-09-08(第30次)に決めた</b>【<b>確度B・一般類型。"
            ...
            "落とした(2026-09-08・第29次 考証方の指摘)。⛔ 飾りを外しはしない。"
...
def zashiki_kazari_check(d):
    ...
    # ⭐ **同じ矛盾に片方だけ気づいている状態を作らない**(2026-09-08・第30次)。
    #   ⛔ 第29次は [西川1959]A の「広間はむしろ遠侍的な性格をもつていた」との逆行に
    ...
    if kz is not None:
        ...
        if inRoof != inKaz:
            bad.append("[西川1959] の室格の記述との逆行の自覚が**片側にしかない** — "
                       ...
                       "⛔ 同じ矛盾に片方だけ気づいている状態を作らない(第29次の指摘)"
...
def _group_points(d, pl, gi):
    ...
    (2026-09-08・第30次)— 実装が据えるのは `scatter_gardens` の点そのもの。"""
    key = (pl["zone"], pl["layer"])
...
def gate_kuguri_check(d):
    """**表門の袖塀の潜り戸が、部材の制約と動線の導出を守っているか**(2026-09-08・第30次)。
...
def zashiki_kazari_men_check(d):
    """**座敷飾を据える壁面の宣言が、棟の作りと矛盾していないか**(2026-09-08・第30次)。
    ...
    for m in d["munes"]:
        ...
        for r in m.get("rooms", []):
            ...
            for k, ja in KAZARI_JA:
                ...
                # ⭐ **床・違棚を『襖の通り』へ据えていないか**(2026-09-08・第30次)。
...
def underscore_note_check(d):
    ...
    (2026-09-08・第30次・普請奉行裁定)。

    ⭕ 規則: **どちらでもない注記は書かない・見つけたら消す。**
    ⛔ 出自(第29次・検図方)— `_` 接頭の注記は生成器が読まず図に一切出ないので、
...
def hedge_pitch_check(d):
    ...
    ⛔ **規則19 の型そのもの。**`groups[].pitch`(芯々 0.8〜1.2 間の不等)は第1次から
...
def hedge_pattern_check(d):
    """**塊の『型』が実出力に出ているか**(2026-09-08・第30次)。
...
def group_region_check(d):
    """**塊が置き場所を二重に宣言していないか**(2026-09-08・第30次)。
...
def roof_kaku_check(d):
    ...
    (2026-09-08・第28次・普請奉行裁定)。⛔ 数値はここに書かない — `roofRule` が正典。"""
    R = d.get("roofRule")
    if not R:
...
def roof_bands_table(d):
    ...
    ⭐ **部材の列**(2026-09-08・第28次)— 帯割りが名指しする FBX が在庫に在るかをここで出す。
    ...
    return ("<h3>屋根の帯割り(帯・桁行・大棟の向き・入側・葺材・棟高・部材・確度)</h3>"
            ...
            "⭐ <b>2026-09-08(第28次・普請奉行裁定=考証方 B案)で帯割りと大棟の向きを引き直した</b>"
            "【確度 P・設計判断。⛔ 史料は無い】 — ①<b>格の頂点(%s)だけが上限 %g間 を超える帯</b>を"
            "持ち、複合内で単独に最も高い(⛔ 従前は帯が 4 と 5 の 2 値しかなく、"
            "<b>最も格の高い大広間が黒書院より低い</b>逆転が起きていた)。"
            "②<b>表向の列と奥向の列で大棟を直交</b>させた(⛔ 従前は 8 棟すべて同じ向きで、"
            "真上から見ると同じ帯の繰り返し=倉庫の姿だった)。規則は `_roofRule`、"
            ...
            "<br>⚠⚠ <b>[二条城二の丸御殿]A との乖離を承知の上で採る</b>(2026-09-07・第23次・"
...
def eave_table(d):
    ...
    ⭐ 2026-09-08(第28次)新設 — ⛔ 従前は出自が `const._*Eave` の長文注記の中にしか無く、
    ...
    return ("<h3>軒高の三段(格を読む軸)</h3><div class='tw'><table><thead><tr>"
            ...
            "<br>⛔⛔ <b>三つとも『他邸と同じ値』という書き方をしていたのを 2026-09-08(第28次)に"
...
def neishi_edge_overhang_check(d):
    """**板塀の根石の厚みが面の縁からはみ出していないか**(2026-09-10・第33次)。
...
def slope_cover_check(d, dem):
    """**低木の地被の被覆率**(Σ 低木の樹冠面積 ÷ 帯の面積)(2026-09-10・第33次)。
...
def _slope_ground_fn(d, dem):
    ...
    ⭐ 2026-09-14(第39次)に検査から切り出した — 散布の透かし(`emergent.thin`)と検査が同じ物差しを使う。"""
    K9 = RGrid(d).ken
    buck = {}
    for q in slope_samples(d, dem):
...
def slope_emergent_check(d, dem):
    """**林冠を抜けて突出する松が `maxGap` ごとに立っているか**(2026-09-10・第33次)。
    ...
    for lay in d.get("slopePlanting", []):
        ...
        # ⭐ **透かし**(2026-09-14 第39次・庭方)— 替えた本数を黙らせない。⛔ 帯W1 は相手に数えない(`_offsets` の宣言)
        th = em.get("thin")
        if th:
...
def torii_route_check(d):
    """**鳥居が参道の線上に載り、正面が参拝者の来る側を向き、柱が参道の外に立つか**(2026-09-14 第38次・庭方 設計 D2 / 第39次)。
    ...
    # ⭐ 社の正面(第39次・検図)— 参道の終点が正面の面に載り、終いの区間が正面へ入るか
...
def slope_doryu_check(d, dem):
    """**法尻の土留め**が在庫の部材で解決しているか・どの断面に要るか(2026-09-10・第33次)。"""
...
def group_mix_check(d):
    """**塊の中で 2 種が混ざっているか**(2026-09-10・第33次【庭方 裁定A-2】)。
...
def main():
    ...
    fig(h, goten_plan(d, -50, 22, -3, 36, "表向 平面",
        ...
        cap="<b>表門 → 白洲 → 石段(4段) → 御式台・御玄関</b>。西へ大広間・黒書院・表役所(藩庁)、"
            ...
            "<br>⭐ <b>据える壁面を 2026-09-08(第30次)から描く</b> — 太い朱線が"
    ...
    fig(h, roof_plan_svg(d),
        legend='<span>━ 大棟</span><span>─ 隅棟・破風</span>'
               ...
               '↓ 水の流れ</span>',
        cap="⭐ <b>2026-09-08(第28次)に、この図を帯割り(<code>munes[].roof.bands</code>)から"
            "描き直した。</b>⛔ 従前は棟の外形の全幅から大棟の高さを出していて、"
            "<b>実装(帯ごとの屋根)より 2.6〜3.7 倍高い、実在しない形</b>を描いていた。"
            "いまは<b>帯ごとに矩形・隅棟・破風・大棟・谷</b>を描くので、"
            ...
            "屋根面が向かい合う(⭐ 2026-09-08 に表向の列の大棟を v へ回したのでここが替わった)。"
    ...
    fig(h, roof_section_svg(d),
        cap="<b>屋根の縦断面(桁行に沿った展開)。</b>"
            "⭐ <b>2026-09-08(第28次)に帯割りから描き直した。</b>"
    ...
    for s in d["sections"]:
        ...
        # ⛔⛔ **2026-09-08(第28次)に断りを復活**(`section_roof_status` の docstring)。
