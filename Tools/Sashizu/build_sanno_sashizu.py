...
def _kakoi_items(d):
    ...
    for k in d["kaidans"]:
        ...
        bands.append(("石段:" + k["name"], [(pt[i], pt[i + 1]) for i in range(len(pt) - 1)],
                      kaidan_wken(d, k) / 2.0))
    # ⭐ **渡廊下の屋根も幅を持つ帯**【検図1巡目 中 → 2026-09-16】── 透塀の南の辺を潜りで抜ける
    #    ので、線 × 帯 の積で離れを測る(⛔ 芯線では測れない)。
...
def noplant_overlap_check(d, g):
    ...
    for gd in d["gardens"]:
        ...
        # ⛔ **照合先の無い『輪郭の外 0 坪』を刷らない**【検図1巡目 中 → 2026-09-16】── 12 件中 10 件が
        #    `clip` 未指定で「平場『—』の輪郭の外 0 坪」と刷っており、**測っていない**ことと
        #    **測って綺麗**なことが区別できなかった(規則19)。⇒ 載る平場を `clip` で名指すか、
        #    平場の外に据わる面は `clipNone` に**理由を書いて**宣言する。
        if gd.get("clip") and te is None:
            bad.append("空地『%s』の `clip`『%s』が `terraces` に無い(指し先の無いポインタ)"
                       % (gd["name"], gd["clip"]))
        if te is None and not gd.get("clipNone"):
            bad.append("空地『%s』が `clip`(載る平場)も `clipNone`(平場の外に据わる宣言と理由)も持たない "
                       "— 照合先が無ければ『輪郭の外 0 坪』は合格ではなく**未測定**(規則19)" % gd["name"])
        ...
        note.insert(0, "空地『%s』 ⭕ ── %.2f 坪(%d セル・刻み %g 間)を全数走査 ── "
                       "%s ／ 低木の帯 %d 本・動線 %d 本と突き合わせ済み"
                       "【算出 — ⛔ 0 件は合格ではなく未測定なので、**何をどれだけ測ったか**を刷る】"
                    % (gd["name"], len(cells) * ct, len(cells), st,
                       ("平場『%s』の輪郭の外 0 坪" % gd["clip"]) if te else
                       ("**平場の外に据わる面**(`clipNone` ── %s)。⛔ 輪郭の照合は対象外"
                        % gd.get("clipNone")),
                       len(bands), len(d.get("routes") or [])))
    return bad, note
...
_OVL_OK = [
    ...
    # ⭐ **2026-09-16 検図1巡目 中 で1件足した。**渡廊下を総当たりへ載せて初めて立った組。
    #    渡廊下は御供所の北面から北へ出るので、**北面に付く犬走りの上を跨いで**通る
    #    (床は御供所の基壇の天端と地盤の間・`bom[渡廊下…]`)。⛔ 平面で重なるのが正しい姿で、
    #    上下の離れは丈が決まってから測る → `_pending`「渡廊下の丈(床高・軒高・棟高)」
    ("渡廊下:L_Gusho", "御供所の犬走り・北"),
]
...
def _ovl_shapes(d, g):
    ...
    # ⭐ **渡廊下(`links`)を載せる**【検図1巡目 中 → 2026-09-16】── ⚠ 旧版の集合は棟・門・区・帯・
    #    石段・塊・玉垣・低木・踏石・点景で、**屋根を持つ渡廊下が一つも入っていなかった**
    #    (御供所の犬走り・北との重なりを誰も測っていなかった)。
    for lk in d.get("links", []):
        corr_item("渡廊下:" + lk["name"], [tuple(lk["from"]), tuple(lk["to"])], float(lk["w"]) / 2.0)
    for gd in d["gardens"]:
        P = garden_poly(gd)
        if not P: continue
        if gd.get("holesFrom"):
            # ⭐ 抜きのある面(砂利敷の雨落ち)── 多角形の早道を使わず判定関数で測る
            H9 = garden_holes(d, gd)
            us = [q[0] for q in P]; vs = [q[1] for q in P]
            out.append((gd["name"].split("(")[0],
                        (lambda P, H9: (lambda p: in_poly(p, P) and not any(in_poly(p, h) for h in H9)))(P, H9),
                        (min(us), min(vs), max(us), max(vs)), None))
        else:
            poly_item(gd["name"].split("(")[0], P)
    for b in d["slopeBands"]:
...
def derive_gaps(d):
    ...
    for w in d["terraceWalls"]:
        ...
        if vf["side"] in ("東", "西"):
            # ⭐ **v に走る石段の袖**(参道の階 ── ユーザー裁定2 2026-09-19)── 通りは芯 ± 半幅で
...
def gokusho_torigai_check(d, g):
    """**御供所の取り合い**【普請奉行の配置 2026-09-15 / 部材方 d2e9de85 / 庭方 2026-09-15】──
    観音堂の北面との離れ・賄の道の末尾が勝手口に載るか・渡廊下の芯が口に載るか・渡廊下の南の端と部材の北の壁の面・
    部材の口の柱間と `munes[御供所].doors`・溝の吐口と蓋石。⛔ 部材と庭方の口の食い違いは `_pending` が持つ間だけ〔記録〕。"""
    bad, note = [], []
    ken = d["const"]["ken"]
    M = dict((m["name"], m) for m in d["munes"])
    m = M.get("御供所")
    if m is None: return (["棟『御供所』が無い"], [])
    doors = dict((q["name"], q) for q in m.get("doors") or [])
    k = M.get("観音堂")
    if k:
        gap = m["v0"] - (k["v0"] + k["dv"])
        ov = min(m["u0"] + m["du"], k["u0"] + k["du"]) - max(m["u0"], k["u0"])
        line = "御供所の南面 × 観音堂の北面 ── 離れ **%.2f 間 = %.3f m**(東西の重なり %.2f 間)【算出】" % (gap, gap * ken, ov)
        (bad if gap < 1.0 - 1e-6 and ov > 0 else note).append(line + (" ── ⛔ 1 間を割る" if gap < 1.0 - 1e-6 and ov > 0 else ""))
    rt = next((q for q in d.get("routes", []) if q.get("tailEndFrom", {}).get("mune") == "御供所"), None)
    dk = doors.get("勝手口")
    if rt and dk:
        e = rt["pts"][-1]
        eu, ev = g.U(e[0]), g.V(e[1])
        if not (dk["u"][0] - 1e-6 <= eu <= dk["u"][1] + 1e-6):
            bad.append("賄の道の末尾 u %.3f が御供所の勝手口 u %.2f〜%.2f に載らない" % (eu, dk["u"][0], dk["u"][1]))
        else:
            note.append("賄の道の末尾 (u %.3f, v %.3f) ── 勝手口 u %.2f〜%.2f の内・南面の手前 %.3f 間【算出】" % (eu, ev, dk["u"][0], dk["u"][1], m["v0"] - ev))
    lk = next((q for q in d.get("links", []) if q["name"] == "L_Gusho"), None)
    dl = doors.get("渡廊下の口")
    row = next((b for b in d["bom"] if b.get("部材") == "御供所(供の棟)"), {}) or {}
    cu, cv = m["u0"] + m["du"] / 2.0, m["v0"] + m["dv"] / 2.0
    if lk and dl:
        c = lk["from"][0]
        if not (dl["u"][0] - 1e-6 <= c - lk["w"] / 2.0 and c + lk["w"] / 2.0 <= dl["u"][1] + 1e-6):
            bad.append("渡廊下 L_Gusho(u %.3f ± %.2f)が御供所の渡廊下の口 u %.2f〜%.2f に収まらない" % (c, lk["w"] / 2.0, dl["u"][0], dl["u"][1]))
        fz = ((row.get("faceZM") or {}).get("北面") or {})
        if fz.get("wall") is None:
            bad.append("部材の北の壁の面 `bom[御供所(供の棟)].faceZM.北面.wall` が無い ⇒ 渡廊下の南の端は**未測定**")
        else:
            wv = cv + fz["wall"] / ken
            note.append("渡廊下の南の端 v %.4f ／ 御供所の北の壁の面 v %.4f(柱の外面 %.4f・基壇の面 %.4f)── 木口を寄せる量 **%.3f m**(`joints`)【算出】"
                        % (lk["from"][1], wv, cv + fz["colOuter"] / ken, cv + fz["kidan"] / ken, (wv - lk["from"][1]) * ken))
    pend = d.get("_pending") or {}
    for face, nm in (("北面", "渡廊下の口"), ("南面", "勝手口")):
        px = ((row.get("doorsM") or {}).get(face) or {}).get("x")
        dq = doors.get(nm)
        if not px or not dq: continue
        pu = (round(cu + px[0] / ken, 3), round(cu + px[1] / ken, 3))
        if abs(pu[0] - dq["u"][0]) < 0.02 and abs(pu[1] - dq["u"][1]) < 0.02:
            note.append("御供所の%s ── 部材の口 u %.3f〜%.3f = 指図の口【算出】" % (nm, pu[0], pu[1]))
        else:
            ref = next((q for q in pend if q.startswith("御供所の勝手口の柱間")), None)
            line = "御供所の%s ── 部材の口 u %.3f〜%.3f ≠ 指図の口 u %.2f〜%.2f" % (nm, pu[0], pu[1], dq["u"][0], dq["u"][1])
            (note if ref else bad).append(line + (" ── ⚠ 差し戻し中 → `_pending`「%s」" % ref if ref else " ── ⛔ `_pending` に項が無い"))
    te = terrace_poly_uv(d["terraces"][0])
    for dr in d.get("drains", []):
        v = dr["v"]; u0, u1 = dr["u"]
        ln = abs(u1 - u0) * ken
        ...
        for r9 in d.get("routes", []):
            pts = [(g.U(q[0]), g.V(q[1])) if r9.get("world") else (q[0], q[1]) for q in r9["pts"]]
            for i in range(len(pts) - 1):
                if _seg_cross((u0, v), (u1, v), pts[i], pts[i + 1]): lids += 1
        ou = dr["outlet"]["u"]
        ins = in_poly((ou, v), te)
    ...
    for gd in d["gardens"]:
        ...
        if gd.get("chuteFrom"):
            # ⭐ **縦断は『上端 = 平場の設計面』から測る**【検図1巡目 高 → 2026-09-16】。
            #    ⛔ 旧版は両端とも造成前の地盤で刷り、落差を 0.19 m(実は 1.6 m)と読ませていた。
            P9 = garden_poly(gd)
            us = [q[0] for q in P9]; vs = [q[1] for q in P9]
            vc = (min(vs) + max(vs)) / 2.0
            u0c, u1c = max(us), min(us)
            _u1, y0c, y1c = chute_toe(d, gd, u0c, vc)
            ydem0 = dem_h(*g.W(u0c, vc))
            bf9 = float(d["const"]["batterFill"])
            note.append("『%s』── u %.3f(平場の縁)→ %.3f(**法尻**)・長さ %.3f 間 = %.3f m・幅 %.2f 間 ／ "
                        "**上端 = 平場の設計面 %.2f m**(⛔ 同じ点の造成前の地盤 %s m ではない)→ 法尻の地盤 %.2f m ⇒ "
                        "**落差 %.3f m** ／ 法面 1:%.1f(`const.batterFill`)・刻み %.3f 間で解いた ／ 据え石 %s"
                        "【算出 — 上端は `terraces[Keidai].y`・地盤は造成前の正本 P】"
                        % (gd["name"], u0c, u1c, u0c - u1c, (u0c - u1c) * ken, max(vs) - min(vs),
                           y0c, ("%.2f" % ydem0) if ydem0 is not None else "—", y1c, y0c - y1c, bf9,
                           CHUTE_STEP_KEN,
                           ("(%.3f, %.3f)" % tuple(gd["sueishi"]["uv"])) if (gd.get("sueishi") or {}).get("uv") else "—"))
            if abs(_u1 - u1c) > 1e-6:
                bad.append("『%s』の下端 u %.3f が法尻 u %.3f と食い違う(%.3f m)── 据え石と水叩きが法面の途中に乗る"
                           % (gd["name"], u1c, _u1, abs(_u1 - u1c) * ken))
            # ⛔ 水叩きは法尻の**外**に据わるか(法面の上に玉石を敷かない)
            for q9 in d["gardens"]:
                if (q9.get("polyFrom") or {}).get("chute") != gd["name"]: continue
                Pm = garden_poly(q9)
                ue = max(q[0] for q in Pm)
                if abs(ue - u1c) < 1e-6:
                    note.append("『%s』の東の縁 u %.3f ≡ 張り石の法尻 ── 法尻の外(西)へ %.3f m × %.3f m【算出】"
                                % (q9["name"], ue, (ue - min(q[0] for q in Pm)) * ken,
                                   (max(q[1] for q in Pm) - min(q[1] for q in Pm)) * ken))
                else:
                    bad.append("『%s』の東の縁 u %.3f が張り石の法尻 u %.3f に付いていない(%.3f m)"
                               "── 法面の上に玉石が乗る" % (q9["name"], ue, u1c, abs(ue - u1c) * ken))
    return bad, note
...
def run_height_m(d, r):
    """囲いの**丈**[m・座(足元)から]と、その典拠。図のどこにも無ければ (None, 理由)。

    ⭐ **2026-09-16 検図1巡目 中 で起こした。**門の外形の食い込み検査が7件とも
    「囲いの丈が図に無い」で止まっていたが、**袖塀も透塀も板塀も柵も、丈は図の中に在った**
    (⛔ 未測定と刷りながら測れる物を測っていなかった)。当たる先は四つ ──
    ① `runs[].bom` が指す部材の `outlineM` の最上(袖塀)② 透塀は `bom[透塀…].heightsM.muneTop`
    ③ 板塀は `const.itabeiHM`(部材の実測)④ 柵は `runs[].hFrom` が指す庭の欄。
    ⛔ 丈を数でここに書かない ── どれも図が既に持っている値からの従属値。
    """
    B = dict((b["部材"], b) for b in d["bom"])
    if r.get("bom") and B.get(r["bom"]):
        ol = B[r["bom"]].get("outlineM") or []
        if ol: return max(q["hM"][1] for q in ol), "`bom[%s].outlineM` の最上" % r["bom"]
    if r.get("kind") == "透塀":
        mt = ((B.get("透塀(連子窓の塀)") or {}).get("heightsM") or {}).get("muneTop")
        if mt is not None: return float(mt), "`bom[透塀(連子窓の塀)].heightsM.muneTop`"
    if r.get("kind") == "板塀" and d["const"].get("itabeiHM") is not None:
        return float(d["const"]["itabeiHM"]), "`const.itabeiHM`(板塀の部材の実測)"
    hf = r.get("hFrom") or {}
    if hf.get("garden") and hf.get("field"):
        o = next((q for q in d["gardens"] if q["name"] == hf["garden"]), None)
        for k in hf["field"].split("."):
            o = o.get(k) if isinstance(o, dict) else None
        if o is not None:
            return float(o), "`gardens[%s].%s`" % (hf["garden"], hf["field"])
    return None, "丈が図のどこにも無い"
                    ...
                    for t0, t1 in ivs:
                        ...
                        if kind == "囲い" and o.get("footStep") and run_foot_at(d, o, qm) is not None:
                            ...
                            if ylo < ft9 - EPS and yhi > ft9 + EPS:
                                ...
                                h9, src9 = run_height_m(d, o)
                                if h9 is None:
                                    note.append("⚠ 門『%s』の外形〔%s〕が囲い『%s』の平面を %.3f m 覆う(帯の下端 %.2f ≥ 足元 %.2f)"
                                                "── %s ので当たるかは**未測定**(⛔ 合格ではない)"
                                                % (gt["name"], bd["band"], o["name"], ln, ylo, ft9, src9))
                                elif ylo >= ft9 + h9 - EPS:
                                    note.append("⭕ 門『%s』の外形〔%s〕は囲い『%s』の天端 %.2f(足元 %.2f ＋ 丈 %.3f m ── %s)の"
                                                "**上 %.3f m** を通る(平面では %.3f m 覆う)【算出 — 検図1巡目 中 2026-09-16】"
                                                % (gt["name"], bd["band"], o["name"], ft9 + h9, ft9, h9, src9, ylo - (ft9 + h9), ln))
                                elif (o.get("endFrom") or {}).get("gate") == gt["name"]:
                                    note.append("⚠ 門『%s』の外形〔%s〕は囲い『%s』の丈の内(足元 %.2f 〜 天端 %.2f ── %s)で %.3f m 覆う "
                                                "── **木口の欠き込みで受ける**(`bom[].axis.footAt.notchM` ── 検査『袖塀の部材と楼門・回廊の取り合い』が測る)【算出】"
                                                % (gt["name"], bd["band"], o["name"], ft9, ft9 + h9, src9, ln))
                                else:
                                    hit += 1
                                    bad.append("門『%s』の部材の外形〔%s〕が囲い『%s』の丈の内(足元 %.2f 〜 天端 %.2f ── %s)へ **%.3f m** 食い込む"
                                               % (gt["name"], bd["band"], o["name"], ft9, ft9 + h9, src9, ln))
                        elif kind == "囲い":
                            seat = o.get("seat")
                            if seat is None or ylo < float(seat) - EPS:
                                ...
                                if h9 is None:
                                    note.append("⚠ 門『%s』の外形〔%s〕が囲い『%s』の平面を %.3f m 覆う(帯の下端 %.2f ≥ 座 %.2f)"
                                                "── %s ので当たるかは**未測定**(⛔ 合格ではない)"
                                                % (gt["name"], bd["band"], o["name"], ln, ylo, float(seat), src9))
                                elif ylo >= float(seat) + h9 - EPS:
                                    note.append("⭕ 門『%s』の外形〔%s〕は囲い『%s』の天端 %.2f(座 %.2f ＋ 丈 %.3f m ── %s)の"
                                                "**上 %.3f m** を通る(平面では %.3f m 覆う)【算出 — 検図1巡目 中 2026-09-16】"
                                                % (gt["name"], bd["band"], o["name"], float(seat) + h9, float(seat), h9, src9,
                                                   ylo - (float(seat) + h9), ln))
                                elif (o.get("endFrom") or {}).get("gate") == gt["name"]:
                                    note.append("⚠ 門『%s』の外形〔%s〕は囲い『%s』の丈の内(座 %.2f 〜 天端 %.2f ── %s)で %.3f m 覆う "
                                                "── **木口の取り合い**(検査『門の側柱の外面に端を突き付ける囲い』が測る)【算出】"
                                                % (gt["name"], bd["band"], o["name"], float(seat), float(seat) + h9, src9, ln))
                                else:
                                    hit += 1
                                    bad.append("門『%s』の部材の外形〔%s〕が囲い『%s』の丈の内(座 %.2f 〜 天端 %.2f ── %s)へ **%.3f m** 食い込む"
                                               % (gt["name"], bd["band"], o["name"], float(seat), float(seat) + h9, src9, ln))
                        else:
                            wx, wz = g.W(qm[0], qm[1])
...
def sukibei_kado_reach(d):
    """**隅部材の外形が脚の向きへ張り出す長さ**[m]【部材方の実測 2026-09-18】── `bom[透塀].kadoOutlineM`
    の最大(ピボット = 隅柱の芯から)。

    ⛔ **これを「脚の到達」と読まない。** 外形の最大は**外の角の隅棟**が届く先で、スパンの木口が
    **座る面**(脚の到達)は層ごとに違う(`bom[透塀].kadoLegM` ── 基壇 < 躯体 < 銅瓦)。
    取り違えると隅の取り合いを十数 mm 甘く見る ⇒ 隙間。どちらを指しているかは検査が刷り分ける。
...
def sukibei_kawara_note(row, cnt, plan=None):
    ...
    for mm in sorted(cnt):
        s = mm / 1000.0
        k = max(1, int(round(s / (mod * nom))))      # ⭐ 生成器と同じ式(`tile_scale`)
        rows.append((s, k, s / (k * mod), s / (k * per), cnt[mm]))
    n9 = sum(r[4] for r in rows)
...
def sukibei_span_check(d, g):
    ...
    if leg is None:
        ...
        legs = row.get("kadoLegM") or {}
        # ⭐ **外形の張り出しと脚の到達を刷り分ける**【指図方 2026-09-18】── 外形の最大は**外の角の
        #   隅棟**で、スパンの木口が座る**脚の到達**より甘い。どちらの意味かを図の上で取り違えると
        #   隅に隙間が空く(⛔ 甘い側で寄せる)。
        top9 = max(float(v) for v in legs.values()) if legs else None
        seat9 = max((float(v) for k, v in legs.items() if "棟" not in k), default=None)
        note.append("隅部材の**外形の張り出し %.3f m**(`bom[%s].kadoOutlineM` の最大 ── 隅柱の芯から)"
                    "%s ⛔ **外形を『脚の到達』と読まない**(甘い側で寄せると隙間になる)。"
                    "⛔ **どちらも隣のスパンから引かない** ── スパン部材は端の型ごとに自分の中で帯を"
                    "引き込んで焼いてあり、`n`/`t`/`c`/`h` の四つとも割り付けの差し引きは **0**"
                    "【部材方の実測 2026-09-18 ── K085。2026-09-17 の『食い込みを引く』は二重の引き算で撤回】"
                    % (max(leg), SUKIBEI_ROW,
                       "" if seat9 is None else
                       "。⭐ スパンの木口が**座る面**(脚の到達 `bom[%s].kadoLegM`)は層ごとに %s ── "
                       "最も遠い層でも外形より **%.0f mm 手前**%s。"
                       % (SUKIBEI_ROW,
                          " / ".join("%s %.4g m" % (k, float(v)) for k, v in legs.items()),
                          (max(leg) - seat9) * 1000.0,
                          "" if top9 is None else "(外形の最大は `%s` %.4g m の丸め)"
                          % (max(legs, key=lambda k: float(legs[k])), top9))))
        seat = row.get("endSeatM") or {}
        if seat:
            note.append("**四つの端の座面**(`bom[%s].endSeatM` ── 部材方の実測)── %s。⇒ "
                        "**割り付けの差し引きは四型とも 0**(節点から節点まで丸ごと等分する)"
                        % (SUKIBEI_ROW, " ／ ".join(
                            "`%s` 引き %s%s" % (
                                k,
                                "なし(四層とも節点ちょうど)" if not any(float(x) for x in (v.get("inset") or {}).values())
                                else "・".join("%s %.4g m" % (a, float(b)) for a, b in (v.get("inset") or {}).items() if float(b)),
                                "" if not (v.get("over") or {}) else
                                "・またぎ " + "・".join("%s ±%.4g m" % (a, float(b)) for a, b in v["over"].items()))
...
def kirimori_svg(d, kan, x0, x1, z0, z1, W=900.0):
    ...
    o.append(T(6, 29, "細枠の枡 = 格子の縁 ─ 壁の控え + 平接ぎ(造成の格子 %g m の節点 ─ "
               "`design_y` の外。面の枡は %d m 格子。⭕ **法面は `design_y` の内**なので面の枡で"
               "塗ってある ─ ⛔ 二重に数えない)" % (IMPL_STEP, stp), fs=10, fill="var(--shu)"))
    ...
    o.append(T(pr.W - 6, yy, "うち**縁の法面**(輪郭から直に 1:%g / 1:%g ─ `design_y` の内なので"
               "上の面の欄に算入済み。⛔ 二重に数えない): %d m²　盛 %.0f m³　切 %.0f m³"
               "(最大の厚み %.2f m ／ 輪郭からの届き 最大 %.1f m ─ 母数は**造成の格子 %g m の節点** "
               "%d 点。⚠ 検査『平場の縁が受けられているか』の『法尻の届き』は母数が**輪郭の半刻みの"
               "歩き**で、刻みが違うので一致しない ─ ⛔ 同じ数として読まない)"
               % (d["const"]["batterFill"], d["const"].get("batterCut", 1.0), cl["slopeArea"],
                  cl["slopeFill"], cl["slopeCut"], cl["slopeMaxRise"], cl["slopeReach"],
                  IMPL_STEP, cl["slopeN"]),
               fs=9.5, anchor="end", fill="var(--dim)"))
    yy += 15
    o.append(T(pr.W - 6, yy, "法尻が上限 `featherCap` %g m で切れた法面の節点 %d ─ ⛔ 上限で切れた "
               "ray は現地形に着いていない(数に出さないと『届いた』と読める)"
               % (cl["capM"], cl["capN"]),
               fs=9.5, anchor="end", fill="var(--shu)"))
    yy += 15
    o.append(T(pr.W - 6, yy, "**残る段**(母数=法面の節点 %d)0.05 m 超 %d 点・最大 %.2f m%s ─ "
               "上限で切れた %d ／ **上限外 %d**(⛔ 原因は `featherCap` ではない)"
               % (cl["slopeN"], cl["footN"], cl["footStep"],
                  ("(%s (%.1f, %.1f)・輪郭から %.1f m)"
                   % (cl["footAt"][2], cl["footAt"][0], cl["footAt"][1], cl["footAt"][3]))
...
def shachi_svg(d, kan="其一"):
    ...
    # 社叢の帯(社地の内側を薄く)
    # ⭐ **凡例は面の宣言から引く**【庭方 ⑥ 2026-09-18 → 裁定1】── ⛔ 図に別の文言を直書きしない
    o.append(T(pr.X(-600), pr.Y(760),
               next((q["name"] for q in d["planes"] if q["name"].startswith("社叢")), "社叢"),
               fs=11, fill="var(--take)"))
...
def chute_svg(d, g, label):
    """**溝の吐口と張り石の縦断**(東西・北を見る)【検図1巡目 高 → 2026-09-16】。

    ⭐ **なぜ要るか。** 平場の縁 28.30 から西の法面へ 1.6 m 落ちる段差を切る図が一面も無く、
    註と検査の数だけで「据え石と水叩きが法面に乗っていない」ことを読ませていた。
    ⛔ 数を持たない ── 縁・法尻・地盤・張り石・水叩きはすべて `gardens`/`drains`/`terraces` からの従属値。
    ⚠ 通常の `sections` は社地の境から境まで写す約束なので、2 m の段差はこの**詳細の一面**で見る。
    """
    ken = d["const"]["ken"]
    gd = next(q for q in d["gardens"] if q.get("chuteFrom"))
    mz = next((q for q in d["gardens"] if (q.get("polyFrom") or {}).get("chute") == gd["name"]), None)
    dr = next(q for q in d["drains"] if q["name"] == gd["chuteFrom"]["drain"])
    P9 = garden_poly(gd)
    u0 = max(q[0] for q in P9); u1 = min(q[0] for q in P9); v = dr["v"]
    y0 = float(d["terraces"][0]["y"])
    yToe = dem_h(*g.W(u1, v))
    uW = (min(q[0] for q in garden_poly(mz)) if mz else u1 - 1.0) - 0.8
    uE = u0 + 1.2
    ys = [dem_h(*g.W(uE - i * 0.05, v)) for i in range(int((uE - uW) / 0.05) + 1)]
    ylo = min([q for q in ys if q is not None] + [yToe]) - 0.8
    yhi = y0 + 1.2
    W, top, bot = 900.0, 26.0, 30.0
    sc = W / ((uE - uW) * ken)
    H = (yhi - ylo) * sc + top + bot
    def X(u): return (uE - u) * ken * sc                      # ⭐ 西が右(u が小さいほど右)
    def Y(y): return top + (yhi - y) * sc
    o = _sv(W, H, label)
    o.append(T(W / 2.0, 14, label, cls="big", anchor="middle"))
    o.append(T(10, 14, "左が東(平場)・右が西(法面)／ 北を見る", fs=10))
    # 造成前の地盤
    gp = [(X(uE - i * 0.05), Y(ys[i])) for i in range(len(ys)) if ys[i] is not None]
    o.append(PL(gp + [(X(uW), Y(ylo)), (X(uE), Y(ylo))], stroke="var(--ink)", sw=1.0,
                fill="var(--ishi)", op=0.18, close=True))
    o.append(PL(gp, stroke="var(--ink)", sw=1.2, dash="4 3"))
    # 平場と法面(設計線)
    o.append(PL([(X(uE), Y(y0)), (X(u0), Y(y0)), (X(u1), Y(yToe))], stroke="var(--shu)", sw=2.0))
    # ⛔ 盛土は**設計線と地盤に挟まれた所だけ**(法尻より西は造成しない)
    o.append(PL([(X(uE), Y(y0)), (X(u0), Y(y0)), (X(u1), Y(yToe))] +
                [(X(uE - i * 0.05), Y(ys[i])) for i in range(len(ys) - 1, -1, -1)
                 if ys[i] is not None and uE - i * 0.05 >= u1 - 1e-9],
                stroke="none", fill=_fill(), op=0.9, close=True))
    # 張り石(法面に貼る)と据え石・水叩き
    o.append(PL([(X(u0), Y(y0)), (X(u1), Y(yToe))], stroke="var(--ishi)", sw=7.0, op=0.85))
    if (gd.get("sueishi") or {}).get("uv"):
        o.append('<circle cx="%.1f" cy="%.1f" r="5.5" fill="var(--ishi)" stroke="var(--ink)" stroke-width="0.8"/>'
                 % (X(u1), Y(yToe) - 4))
    if mz:
        Pm = garden_poly(mz)
        mu0, mu1 = max(q[0] for q in Pm), min(q[0] for q in Pm)
        mp = []
        for i in range(21):
            uu = mu0 + (mu1 - mu0) * i / 20.0
            hh = dem_h(*g.W(uu, v))
            if hh is not None: mp.append((X(uu), Y(hh)))
        if mp:
            o.append(PL(mp, stroke="var(--ishi)", sw=5.0, op=0.85))
            o.append(T((X(mu0) + X(mu1)) / 2.0, mp[len(mp) // 2][1] + 18,
                       "水叩き %.2f m" % ((mu0 - mu1) * ken), fs=10, anchor="middle"))
    # 溝の吐口
    ou = dr["outlet"]["u"]
    o.append(R(X(ou) - 6, Y(y0) - 9, 12, 9, fill="var(--paper)", stroke="var(--ink)", sw=0.9))
    o.append(T(X(ou) + 10, Y(y0) - 12, "溝の吐口(石組の溝)", fs=10))
    # 銘
    o.append(LN(X(u0), Y(y0), X(u0), Y(yToe) + 26, stroke="var(--shu)", sw=0.7, dash="3 3"))
    o.append(LN(X(u1), Y(yToe), X(u1), Y(yToe) + 26, stroke="var(--shu)", sw=0.7, dash="3 3"))
    o.append(T(X(u0) - 4, Y(y0) - 16, "平場の縁 %.2f m(設計面)" % y0, fs=11, anchor="end"))
    o.append(T(X(u1) + 6, Y(yToe) + 40, "法尻 %.2f m(造成前の地盤)" % yToe, fs=11))
    o.append(T((X(u0) + X(u1)) / 2.0, (Y(y0) + Y(yToe)) / 2.0 - 12,
               "張り石 幅 %.2f 間 ／ 法面 1:%.1f" % (gd["chuteFrom"]["widthKen"], d["const"]["batterFill"]),
               fs=11, anchor="middle", fill="var(--shu)"))
    o.append(LN(X(u1) - 22, Y(y0), X(u1) - 22, Y(yToe), stroke="var(--ink)", sw=0.8))
    o.append(T(X(u1) - 26, (Y(y0) + Y(yToe)) / 2.0, "落差 %.2f m" % (y0 - yToe), fs=11, anchor="end"))
    o.append(T(X(u0) + 8, Y(ylo) + 14, "造成前の地盤(破線)・盛土(斜線)", fs=10, anchor="end"))
    o.append(ENDSVG)
    return "".join(o)
...
def keidai_svg(d, u0, u1, v0, v1, title, W=900.0):
    ...
    for gd in d["gardens"]:
        ...
        for H9 in garden_holes(d, gd):             # 砂利敷の抜き = 玉石の雨落ち(庭方 2026-09-15)
            o.append(PL([(lp.X(u), lp.Y(v)) for u, v in H9], close=True, fill="var(--ishi)",
                        stroke="none", op=0.55))
    ...
    # 溝(庭方 2026-09-15)── 石組の溝の通りと吐口
    for dr in d.get("drains", []):
        if dr.get("v") is None: continue
        u0, u1 = dr["u"]
        if not inwin([min(u0, u1), dr["v"]], [max(u0, u1), dr["v"]]): continue
        o.append(LN(lp.X(u0), lp.Y(dr["v"]), lp.X(u1), lp.Y(dr["v"]), stroke="var(--ishi)", sw=2.2))
        o.append(LN(lp.X(u0), lp.Y(dr["v"]), lp.X(u1), lp.Y(dr["v"]), stroke="var(--paper)", sw=0.6, dash="3 2"))
        o.append('<circle cx="%.1f" cy="%.1f" r="3" fill="none" stroke="var(--ishi)" stroke-width="1.2"/>'
                 % (lp.X(dr["outlet"]["u"]), lp.Y(dr["v"])))
...
def wall_near_named(d, g, x, z):
    ...
    best = (None, 1e9)
    for w, (ax, az), (bx, bz) in wall_segs_named(d, g):
        ddx, ddz = bx - ax, bz - az
        L2 = ddx * ddx + ddz * ddz or 1.0
        t = max(0.0, min(1.0, ((x - ax) * ddx + (z - az) * ddz) / L2))
        dd = math.hypot(x - (ax + ddx * t), z - (az + ddz * t))
        if dd < best[1]: best = (w, dd)
    return best
...
def wall_top_at(d, g, w, px, pz):
    ...
    return wall_tg_at(d, g, w, px, pz)[0]
...
def kenpei_bottom_area(d):
    ...
    **作り合い**(本殿と幣殿を継ぐ一間。⛔ 当図は幣殿型なので『石の間』と呼ばない)を
    ...
    # ⭐ **屋根を持つ渡廊下(`links`)も分子に入る**【検図1巡目 中 → 2026-09-16】。
    #    ⚠ 旧版は棟・run・門・井戸屋形だけを数え、本瓦葺の屋根を持つ渡廊下が黙って落ちていた
    #    (⛔ 井戸屋形を「屋根を持つから入れる」で足した基準と非対称)。
    B9 = dict((b["部材"], b) for b in d["bom"])
    a_l = 0.0
    for lk in d.get("links", []):
        row = B9.get(lk.get("bom") or "")
        if row is None:
            bad.append("渡廊下『%s』が `bom` の行を名指していない — 屋根の有無が決まらず建蔽率の分子に入れられない"
                       % lk["name"])
            continue
        if not row.get("屋根"): continue
        a_l += math.hypot(lk["to"][0] - lk["from"][0], lk["to"][1] - lk["from"][1]) * float(lk["w"]) * ken * ken
    ...
    note = ["建蔽率の分子 %.1f m²(棟 %d 件 %.1f ／ 屋根付きの run %.1f ／ 門 %.1f ／ 井戸屋形の軒先 %.1f ／ 渡廊下 %.1f)"
            "── 屋根を持たない役の名簿 %s・落とした棟 %s。⚠ **『作り合い』の屋根だけは【U 設計判断】**(三棟を一続きの屋根で覆うと読んだ帰結で、史料は屋根の有無を言わない【?】・考証6巡目 中1)【算出】"
            % (a_m + a_r + a_g + a_i + a_l,
               len([m for m in d["munes"] if m["yaku"] not in ro]), a_m, a_r, a_g, a_i, a_l,
               ("『" + "』『".join(ro) + "』") if ro else "空(すべての棟が屋根を持つ)",
               ("『" + "』『".join(drop) + "』") if drop else "無し")]
    return a_m + a_r + a_g + a_i + a_l, bad, note
...
def ishizai_color_check(d):
    ...
    # ⭐ **当てた材の実測を名簿の側に刷る**【検図1巡目 低 → 2026-09-16】── ⚠ 旧版は受入値
    #    (設計値)だけを刷り、**当てた材の実測が図のどこにも無かった**(⛔ 外れている実測が
    #    図に残らないと、受入値だけが固まって『測っていない』と区別が付かない・規則19)。
    nmeas = 0
    for c in cols:
        mz9 = c.get("measured")
        if not mz9: continue
        nmeas += 1
        H9, S9, V9 = got.get(c["item"], (None, None, None))
        out9 = []
        if H9 and not (H9[0] <= mz9["hueDeg"] <= H9[1]): out9.append("H %.1f が帯 %g〜%g° の外" % (mz9["hueDeg"], H9[0], H9[1]))
        if S9 is not None and mz9["satPct"] > S9: out9.append("S %.1f > 上限 %g" % (mz9["satPct"], S9))
        if V9 and not (V9[0] <= mz9["valPct"] <= V9[1]): out9.append("V %.1f が %g〜%g の外" % (mz9["valPct"], V9[0], V9[1]))
        ref9 = mz9.get("pendingRef")
        line9 = ("品目『%s』の**当てた材の実測** H %.1f° ／ S %.1f ／ V %.1f%s(`%s` ── %s。物差しは `colorGate.measure`)"
                 % (c["item"], mz9["hueDeg"], mz9["satPct"], mz9["valPct"],
                    ("(V p10–p90 %g〜%g)" % tuple(mz9["valP10P90"])) if mz9.get("valP10P90") else "",
                    mz9["material"], mz9["by"]))
        if not out9:
            note.append(line9 + " ── ⭕ 受入値の内【算出】")
        elif ref9 and ref9 in (d.get("_pending") or {}):
            note.append(line9 + " ── ⚠ **受入値の外**(%s)。直しは部材の巡 → `_pending`「%s」【算出】" % ("・".join(out9), ref9))
        else:
            bad.append(line9 + " ── **受入値の外**(%s)なのに直し先(`measured.pendingRef`)が無い" % "・".join(out9))
    note.append("庭の石の名簿 %d 品目のうち**材の実測を持つ %d 品目**(⛔ 実測の無い %d 品目は『合格』ではなく**未測定**)"
                "【算出 — ⛔ 0 件は合格ではない(規則19)】"
                % (len(cols), nmeas, len(cols) - nmeas))
...
#   ⭐ **2026-09-19 ユーザー裁定1 = 案A** ── 旧『**帯**』(輪郭の外 格子の刻みぶんを**面の高さ**で
#   保つ作法)と、その外を独自の法勾配 1:0.7 で降ろす作法は、**両方とも廃した**。縁は三つしかない:
#     ① **`design_y`** ── 法面は**輪郭から直に** `const.batterFill` 1:1.5 / `batterCut` 1:1.0 で
#        降ろす(⛔ 帯のぶん外へ押し出さない。土量・切盛図・断面と**同じ一つの式**)
...
def _dyc(d, g, x, z):
    """`_design_y_cold` の覚え書き ── ⛔ 平接ぎの八方の当たりで同じ点を何度も冷やし直さない
    (`_design_y_cold` は呼ぶたび `slope_lands` の覚え書きを捨てるので、素で呼ぶと走査が跳ねる)。
...
def wall_collar_at(d, g, x, z):
    """その点の**壁の控え**[m] ── 宣言 `wallCollarM` と **見付高 × `const.batterIshi`** の大きい方。

    ⭐ **一定値で持たない**【考証 中 2026-09-19】── 石垣は勾配 `const.batterIshi` を持つので、
      基部は芯から**見付高 × 勾配**だけ外へ出る。宣言の 0.9 m は見付 4.5 m までしか覆わず、
      回廊の基壇(見付 7 m 級)では控えの外の**平接ぎが石垣の胴へ食い込む**。
    ⛔ 新しい数を作っていない ── 宣言を**下限**として、見付からの従属値で押し上げるだけ。
    """
    base = wall_collar_m(d)
    if base is None: return None
    w, top, glo = wall_near_tg(d, g, x, z)
    if w is None or top is None or glo is None: return base
    return max(base, (top - glo) * float(d["const"].get("batterIshi", 0.0)))


def wall_collar_max(d, g, step=1.0):
    """控えの**上限**[m](= 宣言と『いちばん深い見付 × `batterIshi`』の大きい方)。

    ⛔ 走査の幅を見積もるためだけの数 ── 節点ごとの控えは `wall_collar_at`(規則4)。
    """
    if _CMAX.get("key") == id(d): return _CMAX["v"]
    base = wall_collar_m(d)
    if base is None: return None
    bi = float(d["const"].get("batterIshi", 0.0))
    mx = base
    for w in d["terraceWalls"]:
        for q in wall_samples(d, g, w, step):
            mx = max(mx, q[4] * bi)
    _CMAX["key"] = id(d); _CMAX["v"] = mx
    return mx
...
def shoulder_round_max(d):
    """**法肩の丸みの上限**[m] ── ⛔ 新しい物差しではなく**従属値**。

    枡の中は双一次(= 直線)なので、輪郭(法肩)で実装が読む値は必ず設計面より下がる。落ち込みの
    上限は **√2 × `IMPL_STEP` ÷ (4 × いちばん急な法)** ── 枡の対角を法勾配で割った量の 1/4。
    ⭐ 庭方の設計(2026-09-18)は『法肩の丸み ≤ 0.2 m』と宣言したが、その 0.2 は**刻み 0.25 m の
      格子**で出した数である。当図の格子は **1 m**、実装の地形は **2 m テクセル**(`docs/
      terrain-georef-fix.md`)なので、⛔ 格子だけ 0.25 m へ細かくしても**建つ地面は変わらない**
      ── ⛔ 効かない契約を実装へ増やさない。
    ⇒ ⭕ 閾は**刻みと法からの従属値**で持ち、⛔ 数を直書きしない。実測は検査が毎回刷る。
    """
    bf = min(float(d["const"]["batterFill"]), float(d["const"].get("batterCut", 1.0)))
    return math.sqrt(2.0) * IMPL_STEP / (4.0 * bf)


def _grade_reach(d, g=None):
    """縁の作法の走査が届く幅[m] ── 法面のリーチ(`const.featherCap`)+ 平接ぎの一枡 + 控えの上限。"""
    return d["const"].get("featherCap", 12.0) + GRADE_REACH * math.sqrt(2.0) + (wall_collar_m(d) or 0.0)
...
def _grade_rule(d, g, x, z, cold=True):
    ...
    if on_wall(d, g, near[0], near[1]):
        ...
        #   その分だけ**見付が埋まる**(裁定1 の⛔・実測 3 点 最大 1.20 m)。⇒ 事実と実測を
...
def _impl_org(d):
    """実装が読む格子の原点(`graded_grid` と同じ)。⛔ 別の格子を作らない。"""
    if _IMPLR.get("key") != id(d):
        P = d["polygon"]; cap = d["const"].get("featherCap", 12.0)
        _IMPLR.clear(); _IMPLR["key"] = id(d)
        _IMPLR["x0"] = math.floor(min(p[0] for p in P) - cap)
        _IMPLR["z0"] = math.floor(min(p[1] for p in P) - cap)
        _IMPLR["h"] = {}
    return _IMPLR


def _impl_node(d, g, i, j):
    R = _impl_org(d); H = R["h"]; k = (i, j)
    if k not in H: H[k] = graded_y(d, g, R["x0"] + i * IMPL_STEP, R["z0"] + j * IMPL_STEP)
    return H[k]


def impl_read(d, g, x, z, near=True):
    """**実装がその点へ書く値**(`EdoSannoShaRebuild.Graded`)── 四隅がそろえば双一次、そろわ
    なければ**1 セル以内の最寄り**(`Near`)、それも無ければ触らない(= 現地形のまま)。
    戻り (値 or None, 読み方)。⛔ 図の側で別の読み方を発明しない ── 実装の C# と同じ順で引く。
    """
    R = _impl_org(d)
    fx = (x - R["x0"]) / IMPL_STEP; fz = (z - R["z0"]) / IMPL_STEP
    i = int(math.floor(fx)); j = int(math.floor(fz)); tx = fx - i; tz = fz - j
    c = [_impl_node(d, g, i, j), _impl_node(d, g, i + 1, j),
         _impl_node(d, g, i, j + 1), _impl_node(d, g, i + 1, j + 1)]
    if all(v is not None for v in c):
        return ((c[0] * (1 - tx) + c[1] * tx) * (1 - tz)
                + (c[2] * (1 - tx) + c[3] * tx) * tz, "双一次")
    if not near: return (None, "触らない")
    bi = int(round(fx)); bj = int(round(fz)); best = None
    for dj in (-1, 0, 1):
        for di in (-1, 0, 1):
            v = _impl_node(d, g, bi + di, bj + dj)
            if v is None: continue
            dd9 = di * di + dj * dj
            if best is None or dd9 < best[0]: best = (dd9, v)
    if best is not None: return (best[1], "最寄り")
    return (None, "触らない")
...
def grade_collar_stats(d, g):
    ...
    ⭐ **裁定1(帯の廃止)で帳簿が二つに分かれた**:
      ・**縁の作法**(② 壁の控え + ③ 平接ぎ)は `design_y` に出ない ⇒ 切盛図の欄『格子の縁』が刷る。
        ⭕ **平接ぎは Δ=0**(現地形をそのまま書く)なので**土は動かない** ── それを毎回測って示す
        (⛔ 「動かないはず」を宣言で済ませない)。
      ・**法面**(輪郭から直に 1:`batterFill` / 1:`batterCut`)は `design_y` の内なので、切盛図の
        **面の欄にすでに入っている** ⇒ ⛔ 二重に足さない。**内訳**として別に刷る(庭方の見積りと
        突き合わせられる形 ── 敷き・盛切・最大の厚み・輪郭からの届き)。
    ⚠ 「土留めが受けているか」の閾は `on_wall` の既定(⛔ ここで新しい物差しを作らない)。
    戻り値 dict(add, wallAdd, apronAdd, fillM3, cutM3, maxRise, maxCutD, apronMaxDv, cells,
                slopeN, slopeArea, slopeFill, slopeCut, slopeMaxRise, slopeReach, stairN,
                outN, outArea, outFill, outCut, step, stepAt, why, stepM, wallCollarM)。
    """
    if _COLLAR.get("key") == id(d): return _COLLAR["v"]
    P8 = d["polygon"]
    reach = _grade_reach(d, g)
    a9 = IMPL_STEP * IMPL_STEP
    q = {"add": 0, "wallAdd": 0, "apronAdd": 0, "fillM3": 0.0, "cutM3": 0.0,
         "maxRise": 0.0, "maxCutD": 0.0, "apronMaxDv": 0.0, "cells": [],
         "slopeN": 0, "slopeArea": 0.0, "slopeFill": 0.0, "slopeCut": 0.0,
         "slopeMaxRise": 0.0, "slopeReach": 0.0, "stairN": 0,
         "outN": 0, "outArea": 0.0, "outFill": 0.0, "outCut": 0.0,
         "step": 0.0, "stepAt": None, "why": {}, "stepM": IMPL_STEP,
         # ⭐ **法尻が上限で切れた所**【検図 低 2026-09-19】── `featherCap` は作図の打ち切りで、
         #   そこまでに現地形へ着かない ray は**段を残したまま**終わる。⛔ 数に出ないと「届いた」と
         #   読めてしまうので、上限で切れた節点数と残る段を毎回刷る(規則19)。
         "capM": d["const"].get("featherCap", 12.0), "capN": 0,
         # ⭐ **残る段は原因で分ける**【検図 高 2026-09-19】── 旧図は段の数と最大を
         #   `featherCap` の一文の中に並べ、⛔ 原因を上限と読ませていた。実際に最大の段が立つ
         #   北西(x≈-544・z≈928)は輪郭から 5.5 m で**上限に達していない** ── 原因は
         ...
         "slopeHist": {}, "slopeFar": [],
         # ⭐ **石垣の基部の出と控えの差を測る**【考証 中 2026-09-19】── 控えは一定値 `wallCollarM`
         #   だが、石垣は勾配 `const.batterIshi` を持つので基部は芯から **見付 × 勾配** 出る。
         #   ⛔ 作法をここで変えない(控えの帯には**天端**が書かれるので、広げると見付が埋まる)。
         #   ⭕ **食い込みを毎回測って出す** ── 平接ぎの節点が石の胴の内側に落ちた数と深さ。
         "wallCollarM": wall_collar_m(d), "wallCollarMax": wall_collar_max(d, g),
         "stoneIn": 0, "stoneInMax": 0.0, "stoneInAt": None}
    seen = set()                       # ⛔ 面が二枚届く節点を二度数えない
    for te, P, bb in _terrace_polys(d, g):
        ...
        for j in range(j0, j1 + 1):
            for i in range(i0, i1 + 1):
                ...
                if y is None and w == "縁の作法の外(面から遠い)": continue
                seen.add((i, j))
                nat = dem_h(x, z)
                if y is None:
                    w = w or "説明の付かない欠け"
                    ...
                    if w == "社地の外": q["outN"] += 1
                    continue
                dv = None if nat is None else y - nat
                if kind == "design_y":
                    # ① 法面(と石段)── `design_y` の内なので**面の欄**にもう入っている
                    if _stair_y(d, g, x, z) is not None: q["stairN"] += 1; continue
                    q["slopeN"] += 1; q["slopeArea"] += a9
                    dd9 = _poly_dist(x, z, P)
                    q["slopeReach"] = max(q["slopeReach"], dd9)
                    if dv is not None:
                        if dv > 0: q["slopeFill"] += dv * a9; q["slopeMaxRise"] = max(q["slopeMaxRise"], dv)
                        else:      q["slopeCut"] += -dv * a9
                    if dd9 >= q["capM"] - IMPL_STEP: q["capN"] += 1
                    if dv is not None:
                        b9 = min(6, int(dd9 // 2.0))      # 0-2 / 2-4 / … / 10-12 / 12+
                        h9 = q["slopeHist"].setdefault(b9, [0, 0.0, 0.0, 0.0])
                        h9[0] += 1; h9[1] += abs(dv)
                        h9[2] = max(h9[2], abs(dv)); h9[3] += abs(dv) * a9
                        if dd9 > 6.0:
                            q["slopeFar"].append((round(x, 1), round(z, 1), round(dd9, 1),
                                                  round(dv, 2)))
                    # ⭐ **法尻の残る段** ── 法面の節点の隣が「造成しない(現地形)」か「平接ぎ
                    #   (Δ=0 = 現地形)」なら、その差がそのまま**土の段**として残る。
                    #   ⛔ ②③の節点だけ測って法面を測らないと、上限で切れた ray の段が数に出ない。
                    capped = dd9 >= q["capM"] - IMPL_STEP
                    nb = {}
                    for dx, dz in ((IMPL_STEP, 0), (-IMPL_STEP, 0), (0, IMPL_STEP), (0, -IMPL_STEP)):
                        y2, _w2, k2 = grade_rule(d, g, x + dx, z + dz)
                        if y2 is None:
                            y2 = dem_h(x + dx, z + dz)
                        ...
                        if y - y2 > 0.05:
                            q["footN"] += 1
                            q["footCapN" if capped else "footNearN"] += 1
                        if y - y2 > q["footStep"]:
                            q["footStep"] = y - y2
                            q["footAt"] = (round(x, 1), round(z, 1), te["name"], round(dd9, 1),
                                           capped)
                    # ⭐ **一枡幅の畝** ── 向かい合う二つの隣がどちらも低ければ、その節点は
                    #   幅 1 m の土の畝として残る(⛔ 段の数に紛れさせない)
                    for e1, e2 in (((IMPL_STEP, 0), (-IMPL_STEP, 0)),
                                   ((0, IMPL_STEP), (0, -IMPL_STEP))):
                        if e1 not in nb or e2 not in nb: continue
                        ...
                        break
                    continue
                # ②③ 縁の作法が足した節点 ── ⛔ ここだけが無帳簿になりうる
                q["add"] += 1
                q["wallAdd" if kind == "壁の控え" else "apronAdd"] += 1
                if kind == "平接ぎ":
                    w7, top7, glo7 = wall_near_tg(d, g, x, z)
                    if w7 is not None and top7 is not None and glo7 is not None:
                        so7 = (top7 - glo7) * float(d["const"].get("batterIshi", 0.0))
                        dw7 = _wall_dist(d, g, x, z)
                        if dw7 < so7 - 1e-9:
                            q["stoneIn"] += 1
                            if so7 - dw7 > q["stoneInMax"]:
                                q["stoneInMax"] = so7 - dw7
                                q["stoneInAt"] = (round(x, 1), round(z, 1), w7["name"],
                                                  round(top7 - glo7, 2))
                q["cells"].append((x, z, None if dv is None else round(dv, 3)))
                if dv is not None:
                    if dv > 0: q["fillM3"] += dv * a9; q["maxRise"] = max(q["maxRise"], dv)
                    else:      q["cutM3"] += -dv * a9; q["maxCutD"] = max(q["maxCutD"], -dv)
                    if kind == "平接ぎ": q["apronMaxDv"] = max(q["apronMaxDv"], abs(dv))
                if not in_poly((x, z), P8):
                    q["outArea"] += a9
                    if dv is not None:
                        if dv > 0: q["outFill"] += dv * a9
                        else:      q["outCut"] += -dv * a9
...
def _edge_walk(d, g, stp=None):
    """平場の輪郭を半刻みで歩く ── (面, 点, 外向き, 設計面, 現地形, 土留めが受けるか)。
    ⛔ 縁を測る検査ごとに別の歩き方を作らない(規則4)。"""
    s9 = stp or (IMPL_STEP / 2.0)
    for te, P, bb in _terrace_polys(d, g):
        for i in range(len(P)):
            ax, az = P[i]; bx, bz = P[(i + 1) % len(P)]
            L = math.hypot(bx - ax, bz - az)
            m9 = max(1, int(math.ceil(L / s9)))
            for k in range(m9 + 1):
                t = k / float(m9)
                x, z = ax + (bx - ax) * t, az + (bz - az) * t
                nx, nz = _outward(P, i, x, z)
                yield (te, P, (x, z), (nx, nz), te["y"], dem_h(x, z),
                       on_wall(d, g, x, z), L / m9)


def _stair_edge_walk(d, g, stp=None):
    """**石段の走りの両側**を半刻みで歩く ── (石段, 点, 外向き, 設計面, 現地形, 土留めが受けるか, 長さ)。

    ⭐ **2026-09-19 検図 中** ── 石段の縁は**どの平場の輪郭でもない**ので、`_edge_walk`(平場の
      輪郭を歩く)の母数に一切入らなかった。⇒ 男坂・女坂・参道の階の脇は**どの輪にも入っていない
      未検査**(規則19)。⛔ 別の物差しを作らず、`_edge_walk` と**同じ半刻み**で歩く。
    ⚠ 設計面は `stair_y_at`(= `stair_spans` の割付)そのもの ── ⛔ ここで段を割り直さない。
    """
    s9 = stp or (IMPL_STEP / 2.0)
    for k in d["kaidans"]:
        if k["name"] == "向拝の階": continue
        ...
        for i in range(len(pts) - 1):
            ...
            for j in range(m9 + 1):
                ...
                for sg in (1.0, -1.0):
                    nx, nz = -ez * sg, ex * sg
                    x, z = px + nx * hw, pz + nz * hw
                    top = stair_y_at(k, stair_sfrac(d, g, k, px, pz))
                    yield (k, (x, z), (nx, nz), top, dem_h(x, z),
                           on_wall(d, g, x, z), L / m9)
...
def grade_slope_check(d, g):
    ...
    に仕分ける。⛔ **どちらでもない点が 1 つでも出たら⛔**(法尻が宙に残る)。

    ⛔ **0 件を『合格』と刷らない** ── 何を何点測ったかと、W/F/N/C の延長を必ず添える(規則19)。
    ⛔ **土留めを新設して黙らせない** ── 史料の裏づけが無い(名所図会が石垣を描くのは回廊の基壇・
      坂の側壁・前庭の腰石垣だけ【S】)。受け手が足りなければ**普請奉行の裁定**へ戻す。
    """
    bad, note = [], []
    cap = d["const"].get("featherCap", 12.0)
    bf = float(d["const"]["batterFill"])
    seg = {"W": 0.0, "F": 0.0, "N": 0.0, "C": 0.0}
    n9 = 0; miss = []; over = []; lip = []; reach = 0.0; capn = [0]
    lim9 = shoulder_round_max(d)     # ⛔ 新しい閾を作らない(法肩の丸みと同じ従属値)
    ...
    for te, P, (x, z), (nx, nz), top, nat, walled, dl in _edge_walk(d, g):
        ...
        if walled:
            seg["W"] += dl
            if dv > 0.05:
                w9 = wall_near_named(d, g, x, z)[0]
                if w9 is not None:
                    t9, glo9, _gh9, _bn9 = wall_tg_at(d, g, w9, x, z)
                    mien = t9 - glo9                      # 見付高(裁定 2026-09-09 と同じ定義)
                    wn += 1
                    short = dv - mien
        ...
        # 盛りの縁 ── 法が現地形に着くか(着く距離と、社地の境を跨ぐかを分けて測る)
        t9, ok, inside = 0.0, False, True
        while t9 <= cap + 1e-9:
            t9 += 0.2
            px, pz = x + nx * t9, z + nz * t9
            if inside and not in_poly((px, pz), d["polygon"]): inside = False
            h9 = dem_h(px, pz)
            if h9 is None: break
            if top - t9 / bf <= h9 + 0.05: ok = True; break
        if ok:
            seg["F"] += dl; reach = max(reach, t9)
            if not inside: over.append((round(x, 1), round(z, 1), te["name"], round(t9, 1)))
        else:
            if t9 > cap: capn[0] += 1                     # 上限 `featherCap` で切れた ray
            if dv > lim9:
                miss.append((round(x, 1), round(z, 1), te["name"], round(dv, 2),
                             round(_wall_dist(d, g, x, z), 1)))
            else:
                lip.append((round(x, 1), round(z, 1), te["name"], round(dv, 2)))
    if miss:
        m0 = miss[0]
        bad.append("平場の縁が**受けられていない** ── **%d 点**(例 『%s』 世界座標 (%.1f, %.1f)・"
                   "盛り %.2f m・最寄りの土留めまで %.1f m)。法が社地の内で現地形に着かず、土留めも"
                   "受けていない ⇒ **普請奉行の裁定が要る**(⛔ 史料の裏づけの無い土留めを新設しない)"
                   % (len(miss), m0[2], m0[0], m0[1], m0[3], m0[4]))
    note.append("〔全数当たり〕平場の輪郭 **%.1f m** を半刻み(**%d 点**)で歩いて一点ずつ仕分けた ── "
                "**W 土留めが受ける %.1f m** ／ **F 土羽で降ろす %.1f m**(法 1:%g ── 法尻の届きは"
                "最大 **%.1f m**・母数は**輪郭の半刻みの歩き × 0.2 m の ray**。⚠ 切盛図の"
                "『輪郭からの届き』は母数が**造成の格子 %g m の節点**で、刻みが違うので一致しない"
                "── ⛔ 同じ数として読まない)／ **N 肩のまま触らない %.1f m**"
                "(|設計面 − 現地形| ≤ 0.05 m)／ **C 切りを 1:%g で上げる %.1f m** ／ "
                "**受け手の無い点 %d** ／ 法が上限 `featherCap` %g m で切れた点 **%d**"
                "【算出 ── ⛔ 0 件は合格ではない。母数と延長を必ず添える(規則19)】"
                % (sum(seg.values()), n9, seg["W"], seg["F"], bf, reach, IMPL_STEP, seg["N"],
                   float(d["const"].get("batterCut", 1.0)), seg["C"], len(miss), cap, capn[0]))
    # ⭐ **W の丈の列**(検図 高 2026-09-19)
    if wshort:
        s0 = wshort[0]
        # ⚠ **猶予の指し先が在るあいだは⚠** ── ⛔ 合格ではない(規則18)。項が消えれば⛔ に戻る。
        ref9 = ((d.get("terrainCheck") or {}).get("gradedCover") or {}).get("wallFacePendingRef")
        msg9 = ("**受け手はいるが丈が足りない縁** ── **%d 点**(例 『%s』 世界座標 (%.1f, %.1f)・"
                "段差 **%.2f m** に対し受け手『%s』の見付は **%.2f m** ── **%.2f m が素土のまま**"
                "残る)。⛔ 受け手の**有無**だけを見て合格と読ませない ⇒ **普請奉行の裁定**"
                "(壁を伸ばす／面を退ける／法で降ろす)へ戻す"
                % (len(wshort), s0[2], s0[0], s0[1], s0[4], s0[3], s0[5], s0[6]))
        if ref9 and ref9 in (d.get("_pending") or {}):
            note.append("⚠ " + msg9 + "(→ `_pending`「%s」)【⛔ **猶予であって合格ではない**"
                        "(規則18)── 項が消えれば⛔ に戻る】" % ref9)
        else:
            bad.append(msg9)
    ...
    kn, kmiss, kmax = 0, [], None
    for k9, (x, z), (nx, nz), top, nat, walled, dl in _stair_edge_walk(d, g):
        kn += 1
        if nat is None: continue
        dv = top - nat
        if walled:
            ksg["W"] += dl
            w9 = wall_near_named(d, g, x, z)[0]
            if dv > 0.05 and w9 is not None:
                t9, glo9, _g2, _b2 = wall_tg_at(d, g, w9, x, z)
                short = dv - (t9 - glo9)
                if kmax is None or short > kmax[0]:
                    kmax = (short, round(x, 1), round(z, 1), k9["name"], w9["name"],
                            round(dv, 2), round(t9 - glo9, 2))
            continue
        if dv <= 0.05:
            ksg["C" if dv < -0.05 else "N"] += dl; continue
        t9, ok = 0.0, False
        while t9 <= cap + 1e-9:
            t9 += 0.2
            h9 = dem_h(x + nx * t9, z + nz * t9)
            if h9 is None: break
            if top - t9 / bf <= h9 + 0.05: ok = True; break
        if ok: ksg["F"] += dl
        elif dv > lim9: kmiss.append((round(x, 1), round(z, 1), k9["name"], round(dv, 2)))
        else: ksg["N"] += dl
    if kmiss:
        k0 = kmiss[0]
        bad.append("**石段の縁が受けられていない** ── **%d 点**(例 『%s』 世界座標 (%.1f, %.1f)・"
                   "盛り %.2f m)。⇒ **普請奉行の裁定**へ戻す(⛔ 史料の裏づけの無い側壁を伸ばさない)"
                   % (len(kmiss), k0[2], k0[0], k0[1], k0[3]))
    note.append("〔全数当たり〕**石段の走りの両側**を半刻みで歩いた(**%d 点** ── 男坂・女坂・"
                "参道の階ほか %d 本の敷きの縁。⛔ 石段の縁はどの平場の輪郭でもないので、"
                "`_edge_walk` の母数には入らない)── **W %.1f m** ／ **F %.1f m** ／ **N %.1f m** ／ "
                "**C %.1f m** ／ **受け手の無い点 %d** ／ 側壁の丈の足りなさ 最大 **%+.2f m**%s"
                "【算出 ── ⛔ 未検査を『合格』と読ませない(規則19)】"
                % (kn, len([k for k in d["kaidans"] if k["name"] != "向拝の階"]),
                   ksg["W"], ksg["F"], ksg["N"], ksg["C"], len(kmiss),
                   (kmax[0] if kmax else 0.0),
                   ("(『%s』 世界座標 (%.1f, %.1f)・受け手『%s』・段差 %.2f m ／ 見付 %.2f m)"
                    % (kmax[3], kmax[1], kmax[2], kmax[4], kmax[5], kmax[6])) if kmax else ""))
    if lip:
        l0 = lip[0]
        note.append("受け手が無いが**格子の丸みの内**に収まる縁 ── **%d 点**(例 『%s』 世界座標 "
                    "(%.1f, %.1f)・盛り **%.2f m** ≤ 法肩の丸みの上限 **%.3f m**)。⭕ 法が現地形へ"
                    "届く前に丸みに埋もれる高さなので、**土留めも法面も要らない**(⛔ 数を宣言で"
                    "済ませず毎回測る)【算出】" % (len(lip), l0[2], l0[0], l0[1], l0[3], lim9))
    if over:
        o0 = over[0]
        note.append("法尻が**社地の境を跨ぐ**縁 ── **%d 点**(例 『%s』 世界座標 (%.1f, %.1f)・"
                    "輪郭から %.1f m)。⭕ **造成が社地の外へ出ること自体は裁定済**"
                    "(`_pending`「造成が社地の外へ出ること」・面積と土量は"
                    "検査『造成が社地の外へ及んでいないか』と切盛図の欄『うち社地外』が刷る)。"
                    "⛔ ここで法を急にして黙らせない【算出】"
                    % (len(over), o0[2], o0[0], o0[1], o0[3]))
    q = grade_collar_stats(d, g)
    note.append("縁の法面(輪郭の**外**の `design_y`)── 節点 **%d**(敷き **%d m²**・造成の格子 %g m)／ "
                "盛 **%.1f m³** ／ 切 **%.1f m³** ／ 最大の厚み **%.2f m** ／ 輪郭からの届き 最大 "
                "**%.1f m**。⭕ この土は `design_y` の内なので**切盛図の面の欄にもう入っている** ── "
                "⛔ 二重に数えない(ここは内訳)【算出 ── ⛔ 数を指図の文章に写さない】"
                % (q["slopeN"], q["slopeArea"], IMPL_STEP, q["slopeFill"], q["slopeCut"],
                   q["slopeMaxRise"], q["slopeReach"]))
    # ⭐ **厚みの分布を名簿のまま刷る**【庭方 中2 ／ 考証 中3 2026-09-19】── ⛔ 「遠い節点は
    #   薄いから土量に効かない」という一言で畳まない。輪郭からの距離で刻んで毎回測る。
    hh = q["slopeHist"]
    far = q["slopeFar"]
    note.append("**縁の法面の厚みの分布**(輪郭からの距離で刻む・造成の格子 %g m の節点)── %s"
                "【算出 ── ⛔ 代表値ひとつで『薄いから効かない』と畳まない(庭方 中2)】"
                % (IMPL_STEP,
                   " ／ ".join("**%s m** 節点 %d・厚み 平均 %.2f m / 最大 %.2f m・土 %.1f m³"
                              % (("%d〜%d" % (b * 2, b * 2 + 2)) if b < 6 else "12 以遠",
                                 v[0], v[1] / (v[0] or 1), v[2], v[3])
                              for b, v in sorted(hh.items())) or "—"))
    if far:
        f9 = sorted(far, key=lambda t: -t[3])
        note.append("うち**輪郭から 6 m より外**の節点 **%d**(厚み 0.05 m 以下 %d ／ 0.05〜0.30 m %d ／ "
                    "**0.30 m 超 %d**)── 厚い順に 『(%s)』。⛔ **全部が薄いわけではない** ── "
                    "北の突端は輪郭から 5 m 以上外で設計面が現地形の 1 m 超上に立つ(庭方 中2)"
                    "【算出 ── 名簿そのもの。⛔ 本数を口頭で言い換えない】"
                    % (len(far), len([t for t in far if abs(t[3]) <= 0.05]),
                       len([t for t in far if 0.05 < abs(t[3]) <= 0.30]),
                       len([t for t in far if abs(t[3]) > 0.30]),
                       "／".join("(%.1f, %.1f) 外へ %.1f m・%+.2f m" % t for t in f9[:8])))
    return bad, note


def graded_cover_check(d, g):
    """**造成の格子が面の輪郭を覆い切るか / 法肩の丸み**【K060 2026-09-16 → 裁定1 2026-09-19】。

    ⭐ **測るのは『実装がその点へ書く値』**(`impl_read` ── 双一次 → 最寄り → 触らない)。
      ⛔ 節点の有無だけを数えて『覆えた』と読ませない ── 枡が閉じない所は実装が最寄りで拾うか、
      触らずに現地形を残す。どちらも**設計面との差**になるので、差そのものを測る。
    ⭕ **土の縁**(法面で降ろす縁)の落ち込み = **法肩の丸み**。上限は `shoulder_round_max`
      (刻みと法からの**従属値** ── ⛔ 数を直書きしない)。⛔ 超えたら⛔。
    ⚠ **壁・石段の縁**は設計そのものが**段**(壁の見付・段鼻)なので、⛔ 同じ物差しを当てない ──
      落ち込みは〔記録〕に数として刷り、土で埋めない(⛔ 旧『帯』へ戻さない)。
    """
    dec = (d.get("terrainCheck") or {}).get("gradedCover")
    if not dec or dec.get("wallCollarM") is None:
        return (["格子の縁の作法 `terrainCheck.gradedCover`(と壁の控え `wallCollarM`)の宣言が無い"
                 " — 覆い切れない縁の理由が図に残らない"], [])
    lim = shoulder_round_max(d)
    bad, note = [], []
    cat = {}
    for te, P, (x, z), (nx, nz), top, nat, walled, dl in _edge_walk(d, g):
        v, how = impl_read(d, g, x, z)
        if v is None: v = nat
        if v is None: continue
        key = (te["name"], "壁・石段が受ける縁" if walled else "土の縁(法面で降ろす)")
        q = cat.setdefault(key, [0, 0.0, None, {}])
        q[0] += 1; q[3][how] = q[3].get(how, 0) + 1
        if top - v > q[1]: q[1] = top - v; q[2] = (round(x, 1), round(z, 1), how)
    for (nm, kind), q in sorted(cat.items()):
        if kind.startswith("土") and q[1] > lim + 1e-9:
            bad.append("面『%s』の**法肩の丸み**が上限を超えた ── **%.3f m**(上限 **%.3f m** = "
                       "√2 × 格子の刻み %g m ÷ (4 × 最も急な法)・世界座標 (%.1f, %.1f)・読み方 %s)"
                       % (nm, q[1], lim, IMPL_STEP, q[2][0], q[2][1], q[2][2]))
        note.append("面『%s』の%s ── 輪郭を半刻みで **%d 点**当たり、設計面と実装の読みの差は最大 "
                    "**%.3f m**%s(読み方の内訳 %s)%s【算出】"
                    % (nm, kind, q[0], q[1],
                       "(世界座標 (%.1f, %.1f))" % (q[2][0], q[2][1]) if q[2] else "",
                       "・".join("%s %d" % t for t in sorted(q[3].items())),
                       "／ 上限 **%.3f m**(= √2 × 刻み ÷ 4 × 最も急な法 ── ⛔ 直書きの数ではない)"
                       % lim if kind.startswith("土") else
                       "／ ⚠ ここは**設計そのものが段**(壁の見付・段鼻)なので丸みの上限を当てない"
                       " ── 格子も実装の地形(2 m テクセル)も垂直面を持てず、天端から法尻へは"
                       "**一枡の斜路**で落ちる。⛔ 土で埋めない(旧『帯』)・⛔ 平場を掘って解かない"))
    q9 = grade_collar_stats(d, g)
    note.append("縁の作法が足した節点 **%d** ── **壁の控え %d**(面の高さを壁の外面 **%g m** まで)／ "
                "**平接ぎ %d**(現地形の高さをそのまま ── 動いた土は最大 **%.3f m**・⭕ **0 が正**)。"
                "／ 足した節点が動かす土 盛 **%.1f m³**・切 **%.1f m³**(最大 %.2f / %.2f m)／ "
                "社地の外へ出た縁 **%d m²** ／ 石段の節点 **%d**。断った理由 ── %s"
                "【算出 ── 切盛図の欄『格子の縁』と**同じ集計**(規則4)】"
                % (q9["add"], q9["wallAdd"], q9["wallCollarM"], q9["apronAdd"], q9["apronMaxDv"],
                   q9["fillM3"], q9["cutM3"], q9["maxRise"], q9["maxCutD"], q9["outArea"],
                   q9["stairN"], "・".join("%s %d" % t for t in sorted(q9["why"].items())) or "—"))
    note.append("**石垣の基部の出と壁の控え**(考証 中 2026-09-19)── 控えは宣言 **%g m** の一定値だが、"
                "石垣は勾配 `const.batterIshi` **%g** を持つので基部は芯から **見付 × 勾配** 出る"
                "(実測の上限 **%.2f m** ── 回廊の基壇)。⇒ **平接ぎの節点 %d 点**が石の胴の内側に"
                "落ちている(最大 **%.2f m** 食い込む%s)。⛔ 指図方はここで作法を変えない ── "
                "控えの帯には**天端**が書かれるので広げると**見付が埋まる**(裁定1 の⛔。試すと"
                "検査『格子が一枡より外へ土を持ち出していないか』が 3 点・最大 1.20 m で鳴る)。"
                "⇒ **普請奉行の裁定**(→ `_pending`「壁の控えが石垣の基部の出を覆っていない」)"
                "【算出 ── ⛔ 0 件ではない。数を宣言で済ませない】"
                % (q9["wallCollarM"], float(d["const"].get("batterIshi", 0.0)),
                   q9["wallCollarMax"], q9["stoneIn"], q9["stoneInMax"],
                   ("・『%s』(見付 %.2f m)・世界座標 (%.1f, %.1f)"
                    % (q9["stoneInAt"][2], q9["stoneInAt"][3], q9["stoneInAt"][0],
                       q9["stoneInAt"][1])) if q9["stoneInAt"] else ""))
    st9 = q9["stepAt"]
    note.append("縁の外向きの段差(縁の節点の設計面 − 隣の造成しない節点の現地形)── 最大 **%.2f m**%s"
                "。⚠ **母数は『縁の作法が足した節点 %d』**(②壁の控え + ③平接ぎ)── 切盛図が並べて"
                "刷る**『残る段 最大 %.2f m』は母数が『法面の節点 %d』**で、⛔ 同じ数として読まない"
                "(『届き』の二数と同じ始末・検図 低 2026-09-19)【算出 ── 法面が輪郭から直に"
                "降りるので、この差を測っている場所は**法尻**である】"
                % (q9["step"], "(%s ── 世界座標 (%.1f, %.1f))" % (st9[2], st9[0], st9[1]) if st9 else "",
                   q9["add"], q9["footStep"], q9["slopeN"]))
...
def grade_carry_check(d, g):
    """**格子が一枡より外へ土を持ち出していないか**(= 土留めの見付を埋めていないか)
    【庭方の設計 ③ 2026-09-18 → 裁定1 2026-09-19】。

    ⭐ **庭方の名指し**:「⛔ 見付を `design_y` で測ると埋没が数字に出ない。**`graded_y`(格子へ
      焼く値)で測り直す検査**を立てること」── `design_y` は壁の外に値を持たないので、旧『帯』が
      壁の前へ積んだ土は**どの数にも出なかった**。⇒ ここは**実装が書く値**(`impl_read`)で測る。
    ⭕ 判定は「**壁の外面から一枡より外**で、現地形を持ち上げていないか」── 格子も実装の地形も
      垂直面を持てないので、天端から法尻へ落ちる**一枡の斜路**は避けられない(そこは石垣の駒が
      隠す)。⛔ **それより外へ土を持ち出したら⛔**(= 旧『帯』の再発)。
    ⛔ 0 件を『合格』と刷らない ── 壁ごとの測点と最大の持ち上げを必ず添える。
    """
    wc = wall_collar_m(d)
    if wc is None: return (["壁の控え `terrainCheck.gradedCover.wallCollarM` の宣言が無い"], [])
    wcx = wall_collar_max(d, g)                  # 走査の見積り(⛔ 判定は点ごとの `wall_collar_at`)
    out9 = wc + math.sqrt(2.0) * IMPL_STEP       # ここより外は⛔ 持ち上げない(一枡の斜路の外)
    lim9 = shoulder_round_max(d)                 # ⛔ 新しい閾を作らない(法肩の丸みと同じ従属値)
    near9 = 0; nr9 = [0]
    bad, note, rows = [], [], []
    polys = _terrace_polys(d, g)
    for w in d["terraceWalls"]:
        n9, mx, at, over = 0, 0.0, None, 0
        for a, b in run_segs(w):
            A = g.W(a[0], a[1]); Bq = g.W(b[0], b[1])
            L = math.hypot(Bq[0] - A[0], Bq[1] - A[1])
            m9 = max(1, int(math.ceil(L / 0.5)))
            ex, ez = (Bq[0] - A[0]) / (L or 1.0), (Bq[1] - A[1]) / (L or 1.0)
            nx, nz = -ez, ex
            for k in range(m9 + 1):
                t = k / float(m9)
                x, z = A[0] + (Bq[0] - A[0]) * t, A[1] + (Bq[1] - A[1]) * t
                for sg in (1.0, -1.0):
                    for dist in (out9, out9 + IMPL_STEP, out9 + 2 * IMPL_STEP):
                        qx, qz = x + nx * dist * sg, z + nz * dist * sg
                        if any(in_poly((qx, qz), P2) for _t, P2, _b in polys): continue
                        # ⚠ **どの**土留めから見ても外面の一枡外であること(⛔ この壁からの距離だけで
                        #   切ると、壁どうしが取り合う隅で隣の壁の控えを拾って鳴る)
                        if _wall_dist(d, g, qx, qz) < out9 - 1e-9: continue
                        if any(_poly_dist(qx, qz, P2) <= math.sqrt(2.0) * IMPL_STEP
                               for _t, P2, _b in polys): continue       # 輪郭の一枡の斜路の中
                        # ⚠ **設計の地盤(法面・石段)から一枡以内も除く** ── そこは同じ
                        #   「垂直を持てない格子の一枡の斜路」で、⛔ 壁の埋没とは別の話。
                        s8 = IMPL_STEP
                        if any(_dyc(d, g, qx + ddx, qz + ddz) is not None
                               for ddx, ddz in ((0, 0), (s8, 0), (-s8, 0), (0, s8), (0, -s8),
                                                (s8, s8), (s8, -s8), (-s8, s8), (-s8, -s8))):
                            continue
                        nat = dem_h(qx, qz)
                        v, _how = impl_read(d, g, qx, qz, near=False)
                        vN, _h2 = impl_read(d, g, qx, qz)
                        if nat is not None and vN is not None and vN - nat > lim9: nr9[0] += 1
                        if nat is None: continue
                        # ⚠ **読めない点も測る** ── 四隅がそろわない枡は実装が触らない = 現地形の
                        #   まま。⛔ そこを走査から外すと母数が消え、0 件が『測っていない』になる。
                        if v is None: v = nat
                        n9 += 1
                        if v - nat > mx: mx = v - nat; at = (round(qx, 1), round(qz, 1))
                        if v - nat > lim9: over += 1
                        elif v - nat > 0.05: near9 += 1
        rows.append((w["name"], n9, mx, over, at))
        if over:
            bad.append("土留め『%s』の**外面より一枡外**で格子が現地形を**丸みの上限(%.3f m)を"
                       "超えて**持ち上げている ── **%d 点**(最大 **%.2f m**・世界座標 (%.1f, %.1f))。"
                       "⛔ 見付が埋まる ── 旧『帯』の再発"
                       % (w["name"], lim9, over, mx, at[0], at[1]))
    tot = sum(r[1] for r in rows)
    note.append("〔全数当たり〕土留め **%d 本**の両側を 0.5 m ごと・壁の線から **%.2f m / %.2f m / "
                "%.2f m**(控えの宣言 + 一枡の斜路。⚠ 石垣の**基部の出**は見付 × 勾配で最大 **%.2f m** "
                "── 控えを超える壁があることは `_pending` に記録)の三段で当たった(**%d 点** ── "
                "平場の中・`design_y` の内(法面・石段)・"
                "輪郭から一枡以内は除く)。**丸みの上限 %.3f m を超える持ち上げ %d 点**(= 欠陥)／ "
                "0.05 m 〜 上限の持ち上げ **%d 点**(格子が垂直を持てないぶんの丸み ── ⭕ 許す)／ "
                "最大 **%.2f m**【算出 ── ⛔ `design_y` ではなく**実装が書く値** `impl_read` で"
                "測った(庭方の名指し 2026-09-18 ③)】"
                % (len(rows), out9, out9 + IMPL_STEP, out9 + 2 * IMPL_STEP, wcx, tot, lim9,
                   sum(r[3] for r in rows), near9, max([r[2] for r in rows] or [0.0])))
    note.append("⚠ **実装の `Near`(四隅がそろわない枡を 1 セル以内の最寄りで拾う外挿)を使うと、"
                "同じ当たりで持ち上げが %d 点に増える** ── 図は縁の枡を**平接ぎ**で閉じたので外挿は"
                "要らない。⇒ 実装は**双一次だけ**で読むこと(⛔ `Near` を使うと山肌の節点が一枡ぶん"
                "均され、社叢の斜面が段になる)【算出 ── 棟梁への申し送り】" % nr9[0])
    note.append("壁ごとの最大の持ち上げ ── %s【算出】"
                % "／".join("%s %.2f m(%d 点)" % (r[0], r[2], r[1]) for r in rows))
    # ⭐ **控えの帯そのものを母数に入れる**【検図 中3 2026-09-19】── 上の走査は壁の線から
    #   一枡の斜路の**外**しか見ていないので、埋没が起きる当の帯(控え %g m)が構造的に外れていた。
    #   ⛔ 0 点は緩い条件の証拠でしかない。⭕ 帯の中は「**その壁の天端を超えて盛っていないか**」
    #   で測る(帯の中の地盤が天端にあること自体は控えの設計そのもの ── ⛔ それを欠陥と数えない)。
    bn, bover, bmax, bat = 0, 0, 0.0, None
    for w in d["terraceWalls"]:
        for a, b in run_segs(w):
            A = g.W(a[0], a[1]); Bq = g.W(b[0], b[1])
            L = math.hypot(Bq[0] - A[0], Bq[1] - A[1])
            if L < 1e-9: continue
            ex, ez = (Bq[0] - A[0]) / L, (Bq[1] - A[1]) / L
            nx, nz = -ez, ex
            m9 = max(1, int(math.ceil(L / 0.5)))
            for k in range(m9 + 1):
                x, z = A[0] + ex * (k * L / m9), A[1] + ez * (k * L / m9)
                for sg in (1.0, -1.0):
                    for dist in (wc * 0.5, wc):
                        qx, qz = x + nx * dist * sg, z + nz * dist * sg
                        if any(in_poly((qx, qz), P2) for _t, P2, _b in polys): continue
                        v, _h = impl_read(d, g, qx, qz)
                        if v is None: continue
                        bn += 1
                        # ⚠ **天端は『いちばん近い壁』から採る** ── 地盤をそこへ書いたのは
                        #   `collar_top` で、それも最寄りの壁から採っている。⛔ この走査だけ
                        #   『いま回している壁』の天端で測ると、壁どうしが取り合う隅で
                        #   **隣の高い壁の天端**を欠陥として数える(偽陽性)。
                        w8 = wall_near_named(d, g, qx, qz)[0] or w
                        # ⚠ **比べる天端は『一枡以内の天端の最大』** ── 実装が書ける値は格子の
                        #   節点(刻み `IMPL_STEP`)から来るので、⛔ 天端が走りなりに上がる壁
                        #   (`coping:"stair"` の側壁)で『その点ちょうどの天端』と比べると、
                        #   一枡ぶん先の段の高さを**埋没として数える**(偽陽性)。
                        #   ⛔ 新しい閾を作らない ── 刻みそのものからの従属値である。
                        s7 = IMPL_STEP
                        top8 = max(wall_top_at(d, g, w8, qx + ddx, qz + ddz)
                                   for ddx, ddz in ((0, 0), (s7, 0), (-s7, 0), (0, s7), (0, -s7),
                                                    (s7, s7), (s7, -s7), (-s7, s7), (-s7, -s7)))
                        dvv = v - top8
                        if dvv > bmax:
                            bmax = dvv; bat = (round(qx, 1), round(qz, 1), w["name"])
                        if dvv > lim9: bover += 1
    if bover:
        bad.append("**控えの帯の中**で格子が**その壁の天端を超えて**盛っている ── **%d 点**"
                   "(最大 **%.2f m**・『%s』 世界座標 (%.1f, %.1f))。⛔ 天端の上に土が載る"
                   % (bover, bmax, bat[2], bat[0], bat[1]))
    note.append("〔全数当たり〕**控えの帯そのもの**(壁の線から **%.2f m / %.2f m**・両側 ── "
                "⚠ 石垣の基部は見付 × 勾配で最大 **%.2f m** 出るので、帯の外にも石の胴がある)を "
                "0.5 m ごとに当たった(**%d 点** ── 平場の中は除く。比べる天端は**その点に"
                "いちばん近い壁の、一枡 %g m 以内の天端の最大** ── 天端が走りなりに上がる側壁で"
                "一枡先の段を埋没と数えないため。⛔ 新しい閾ではなく刻みからの従属値)。"
                "**天端を超える盛り %d 点**"
                "(閾は法肩の丸み %.3f m)／ 天端との差の最大 **%+.2f m**%s【算出 ── ⛔ 埋没が"
                "起きる当の帯を走査から外さない(検図 中3 2026-09-19)】"
                % (wc * 0.5, wc, wcx, bn, IMPL_STEP, bover, lim9, bmax,
                   ("(『%s』 世界座標 (%.1f, %.1f))" % (bat[2], bat[0], bat[1])) if bat else ""))
    return bad, note


def slope_planting_check(d, g):
    """**法面の上に立つ木が庭方の規約を守っているか**【庭方の設計 ④ 2026-09-18 → 裁定1 2026-09-19】。

    ⭐ 規約(`planting.slopeRule`)は「法面の上は**下草と常緑照葉低木だけ**／ 松・落葉高木・中木は
      **法尻から外**(動かない地山の上)」。⛔ **落とすのではなく寄せる**(本数は庭方の密度から出る
      数なので、減らして黙らせない)。
    ⭕ **面の出所は `design_y`** ── 法面 = 平場の輪郭の外で `design_y` が値を持つ節点(⛔ 別に面を
      描かない)。⛔ 石段の敷きは法面ではない。
    ⚠ `slopeRule.pending` が在るあいだは⚠(猶予)で通す ── ⛔ **合格ではない**(規則18)。
    """
    rule = (d.get("planting") or {}).get("slopeRule")
    if not rule:
        return (["法面の上の植栽の規約 `planting.slopeRule` の宣言が無い ── 庭方の設計 ④ が図に無い"], [])
    im = json.load(open(IMPL_OUT, encoding="utf-8"))
    ...
    for p9 in P:
        x, z = p9["world"]
        if any(in_poly((x, z), Q) for _t, Q, _b in polys): continue
        if _stair_y(d, g, x, z) is not None: continue
        if _dyc(d, g, x, z) is None: continue
        lay = p9.get("layer") or "?"
        tal[lay] = tal.get(lay, 0) + 1
        if lay in hi: bad9.append((p9.get("name"), round(x, 1), round(z, 1), lay))
    msg = None
    if bad9:
        b0 = bad9[0]
        msg = ("**法面の上に松・落葉高木・中木が立っている** ── **%d 本**(例 『%s』 世界座標 "
               "(%.1f, %.1f)・層 %s)。規約は『法尻から **%g m** 外』 ⇒ **寄せる**"
               "(⛔ 落とさない ── 本数は庭方の密度から出る数)"
               % (len(bad9), b0[0], b0[1], b0[2], b0[3], rule.get("highTreeFromToeM")))
    note = []
    note.append("〔全数当たり〕撒いた木 **%d 本**の幹を全部当たり、**法面(平場の輪郭の外で "
                "`design_y` が値を持つ所)の上に立つ木 %d 本**を層ごとに数えた ── %s。"
                "規約(`planting.slopeRule`)が法面の上に許すのは **%s**(樹種 %s・丈 %g〜%g m・"
                "芯々 %g 間・%s)、松・落葉高木・中木は**法尻から %g m 外**"
                "【算出 ── ⛔ 0 件は合格ではない。母数を必ず添える(規則19)】"
                % (len(P), sum(tal.values()),
                   "・".join("%s %d 本" % t for t in sorted(tal.items())) or "—",
                   "／".join(rule.get("onSlope") or []), "・".join(rule.get("species") or []),
                   (rule.get("hM") or [0, 0])[0], (rule.get("hM") or [0, 0])[1],
                   rule.get("spacingKen"), rule.get("stagger"), rule.get("highTreeFromToeM")))
    if msg and rule.get("pending"):
        note.append("⚠ **猶予** ── " + msg + "。⛔ **合格ではない**(規則18)── 撒きの面を変えれば"
                    "**本数(密度 × 有効面)も林縁の見え**も動くので、⛔ 指図方は撒き直さない。"
                    "裁定は `_pending`「法面の上に立つ高木・中木を法尻の外へ寄せる」"
                    "(**庭方**と**普請奉行**)【算出】")
        return ([], note)
    return (([msg] if msg else []), note)
...
def impl_planting_check(d, g):
    ...
    note.append("木の足元の出所 ── %s(⛔ 現地形のままの y を焼かない ── 実装は造成した地形に"
                "置く)。**造成した地盤の上に立つ木 %d 本**%s【算出 — 中 庭方2巡目 "
                "2026-09-18 の結線。⛔ 是非は決めない ── 法面の上に立ってよい層は**庭方の規約**"
                "(`planting.slopeRule`)、縁の法そのものは `const.batterFill` / `batterCut`】"
...
_PROBE_CACHES = ("_BANDS", "_BSTAT", "_VCUT", "_GRP", "_VH", "_LAND", "_EDGE_NOTE",
                 "_WSEG", "_WSEGP", "_WCOMP", "_STAIR_CF", "_STAIR_CF_N", "_SCAT_N", "_SITE_EDGE_N",
                 "_GPOLY", "_COLLAR", "_CTOP")
_PROBE_SLOTS = ("_WSEG", "_WSEGP", "_WCOMP", "_SCAT_N")   # `[None]` の一枠 ── 退避中は `[None]` に戻す
...
def run_checks():
    ...
    **名と件数の一覧を必ず全本刷る**(規則19)。戻り値 (⛔止める, 〔記録〕, 一覧)。
    ...
    gsl = grade_slope_check(d, g)    # 平場の縁が受けられているか(ユーザー裁定1 2026-09-19)
    gca = grade_carry_check(d, g)    # 格子が一枡より外へ土を持ち出していないか(庭方③ → 裁定1)
    ipc = _gated(impl_planting_check)  # 撒いた木の面と離れ(2026-09-08)
    slp = _gated(slope_planting_check)  # 法面の上の植栽の規約(庭方 ④ 2026-09-18)
    #   ⛔ `spc`(袖塀の部材の検査)と**同じ名を使わない** ── 二重代入で袖塀の行が別の検査の
    #      結果を刷っていた(考証方 高1 2026-09-19)。名乗る物を測っていない行を載せない。
    ...
    rows.append(("焼き出しの造成後の地盤と**世界座標**が図の算出と一致するか",
                 igc[0], igc[1]))
    rows.append(("造成の格子が面の輪郭を覆い切るか・**法肩の丸み**(実装が書く値と設計面の差)",
                 gcc[0], gcc[1]))
    rows.append(("平場の縁が受けられているか(W 土留め / F 土羽 ── 受け手の無い縁は⛔。"
                 "ユーザー裁定1 2026-09-19)", gsl[0], gsl[1]))
    rows.append(("格子が**一枡より外へ土を持ち出していないか**(= 土留めの見付を埋めていないか ── "
                 "⛔ `design_y` ではなく実装が書く値で測る)", gca[0], gca[1]))
    rows.append(("焼き出した木が宣言した面と離れを守っているか(勝手道・芯々・塊・林縁・"
                 "石段・囲い・`scaleXZ`)", ipc[0], ipc[1]))
    rows.append(("法面の上に立つ木が庭方の規約を守っているか(`planting.slopeRule` ── "
                 "下草と常緑低木だけ・高木中木は法尻の外)", slp[0], slp[1]))
    ...
    rows.append(("庭の石の名簿の色(`ishizai.colors`・受入値 `colorGate`・当てた材の実測 `colors[].measured`)", izc[0], izc[1]))
...
def main():
    ...
    fig(h, chute_svg(d, g, "%s 附図　溝の吐口と張り石の縦断(東西)" % KAN[n[0] - 1]),
        cap="<b>附図 溝の吐口と張り石の縦断。</b>御供所の南の溝が平場の西の縁で吐き、"
            "<b>張り石で法面を下って法尻の据え石・水叩きへ落ちる</b>【U 庭方 2026-09-15】。"
            "⭐ <b>上端は平場の設計面</b>で、造成前の地盤ではない ── "
            "<b>下端は法尻</b>(縁から <code>const.batterFill</code> の法面が造成前の地盤に当たる点)からの従属値で、"
            "⛔ 数を図にも文にも持たない(数は検査『御供所の取り合い』が刷る)。"
            "⚠ この面は社地の境まで写す <code>sections</code> の一本ではなく、"
            "<b>2 m の段差を読むための詳細</b>(検図1巡目 高 — 段差を切る断面が一面も無かった)。")
