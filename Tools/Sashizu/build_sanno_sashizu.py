...
def tamagaki_edges(gd):
    """玉垣の辺 ── (名, a, b, 立てるか, 長さ[間])。⛔ 延長を json に書かない(辺からの従属値)。"""
    tg = gd.get("tamagaki")
    ...
    for e in tg["edges"]:
        ...
        out.append((e["name"], a, b, bool(e.get("fence")), math.hypot(b[0] - a[0], b[1] - a[1])))
    return out


def tamagaki_bays(gd, ei):
    """辺 ei の柱の位置(uv)── **隅は必ず柱**、辺の中は芯々 `postPitchKen` 以下で等分する。"""
    P = [(q[0], q[1]) for q in (gd.get("poly") or [])]
...
def tamagaki_stats(gd):
    """玉垣の 延長[間] と 柱の数 ── **どちらも辺と芯々からの従属値**。

    ⚠ 2026-09-06 検図4巡目 中4: 旧式は辺ごとに `floor(辺長/芯々)+1` を足しており、
    隅を共有する辺の柱を数え落としていた(5辺の島で 13本 → 12本)。
    **隅柱(立てる辺の端点の集合)+ 各辺の中間柱**として数える。
    """
    eg = tamagaki_edges(gd)
    if not eg: return None
    L = sum(q[4] for q in eg if q[3])
    corners, mid = set(), 0
    for e in gd["tamagaki"]["edges"]:
        if not e.get("fence"): continue
        bays = tamagaki_bays(gd, e["i"])
...
def kido_bay_check(d):
    ...
    for gd in d["gardens"]:
        ...
        kd = tg.get("kido")
        if not kd: continue
        bays = tamagaki_bays(gd, kd["edge"])
        a = bays[0]
        ln = math.hypot(bays[-1][0] - a[0], bays[-1][1] - a[1]) or 1.0
        def s(p): return ((p[0] - a[0]) * (bays[-1][0] - a[0]) + (p[1] - a[1]) * (bays[-1][1] - a[1])) / ln
        hw = kd["wM"] / 2.0 / d["const"]["ken"]
        s0, s1 = s(kd["uv"]) - hw, s(kd["uv"]) + hw
        posts = [s(p) for p in bays]
        hit = [i for i, q in enumerate(posts) if s0 - 1e-9 < q < s1 + 1e-9]
        if hit:
            bad.append("%s の%s(芯 %.4f・幅 %.2f m)が柱に乗る — 柱の位置 %s"
                       % (gd["name"], kd["name"], kd["uv"][0], kd["wM"],
                          "・".join("%.4f" % posts[i] for i in hit)))
    return bad


def island_shrubs(gd, ken):
    """植込みの島の照葉低木の位置 ── **面積からの従属値**。⛔ 本数を json に書かない。

    玉垣の芯線から**芯々の半分**だけ内へ寄せた面を、芯々の格子で走査する。
    """
    sh = gd.get("shrubs")
    P = [(q[0], q[1]) for q in (gd.get("poly") or [])]
    ...
    kr = kido_rect(gd, ken)
    out = []
    for q in poly_scan(P, st):
        if min(_pt_seg(q, P[i], P[(i + 1) % len(P)]) for i in range(len(P))) < ins: continue
        if kr and in_poly(q, kr): continue
        out.append(q)
    return out
    ...
    for gd in d["gardens"] + d["slopeBands"]:
        ...
        for sb in shrub_segments(gd):
            ...
            o.append(T(lp.X(sb["u"] - sb["widthKen"] / 2.0),
                       ...
                       fs=9, anchor="middle", fill="var(--take)"))
        # 玉垣(立てる辺は実線・立てない辺は破線)と木戸の開口、島の照葉低木
        _tge = tamagaki_edges(gd)
        for nm, a, b, fen, L in _tge:
            if not inwin(a, b): continue
            o.append(LN(lp.X(a[0]), lp.Y(a[1]), lp.X(b[0]), lp.Y(b[1]),
                        stroke="var(--hei)" if fen else "var(--take)",
                        sw=2.2 if fen else 0.8, dash=None if fen else "3 3"))
        # ⭐ 銘「玉垣」— 図中に一度も出ていなかった(2026-09-06 検図4巡目 低14)
        _fe = [q for q in _tge if q[3] and inwin(q[1], q[2])]
        if _fe:
            _a, _b = _fe[0][1], _fe[0][2]
...
def crown_of(d, kind, prefab, h):
    ...
    xz = (d["planting"]["scaleRule"].get(kind) or {}).get("scaleXZ") or [1.0, 1.0]
...
def avoid_shapes(d, g, scope):
    ...
    scope="zentei" … 前庭の面(植込みの島)に効く(`planting.clearance.zentei`)
    ...
    if scope == "keidai":
        ...
        for k in d["kaidans"]:
            rr = (k.get("wKen") or k["w"] / ken) / 2.0 + ck["kaidanHalfPlus"]
            pts = [tuple(q) for q in (k.get("pts") or [k["a"], k["b"]])]
            for i in range(len(pts) - 1):
                out.append(_shape_seg(pts[i], pts[i + 1], rr, "石段:" + k["name"]))
        return out
    if scope == "zentei":
        cz = d["planting"]["clearance"]["zentei"]
        for rt in d.get("routes", []):
            rr = rt["w"] / 2.0 / ken + cz["routeHalfPlus"]
            pts = [(g.U(q[0]), g.V(q[1])) if rt.get("world") else (q[0], q[1]) for q in rt["pts"]]
            for i in range(len(pts) - 1):
                out.append(_shape_seg(pts[i], pts[i + 1], rr, "動線:" + rt["name"]))
        for k in d["kaidans"]:
            rr = (k.get("wKen") or k["w"] / ken) / 2.0 + cz["kaidanHalfPlus"]
            pts = [tuple(q) for q in (k.get("pts") or [k["a"], k["b"]])]
            for i in range(len(pts) - 1):
                out.append(_shape_seg(pts[i], pts[i + 1], rr, "石段:" + k["name"]))
        p = cz["gate"]
    ...
    rr = (on.get("wKen") or on["w"] / ken) / 2.0 + av["onnazakaFromShoulder"]
    ...
    for i in range(len(zp)):
        ...
        out.append(_shape_seg(a, b, av["zenteiFromEdge"], "前庭の縁"))
    return out
...
def overhang_range(d):
    """参道の林縁が道へ張り出す量[m] — **樹冠からの従属値**。引けなければ None。"""
...
def plant_budget(d, g):
    ...
    for key in ("松", "落葉", "中木", "低木"):
        ...
        det = all(pt.get("detail") for pt in pal[key])
        rows.append((key, int(round(n[key])), avg, det))
    return rows
...
def crown_rule_check(d):
    """(e) **高木の枝下**が `plantRule.crownRule.takagi.edaShitaMinM` 以上か。
    ...
    for gd in d["gardens"] + d["slopeBands"]:
        for sg in gd.get("singles", []):
            ...
            if lo is not None and e is not None and e < lo:
                bad.append("%s の枝下 %.2f m が下限 %.2f m を割る" % (sg["name"], e, lo))
    # ⭐ **中木・低木は樹冠の外周で退避を測る規約**(`crownRule.roles`)なのに、一本立ちには
    #    当たっていなかった(2026-09-06 検図4巡目 高3)。⛔ **止めない** — 本数を減らすか部材を
    #    替えるか `scaleXZ` を下げるかは庭方の意匠。〔記録〕として毎回刷る(→ `_pending`)。
    g = G(d)
    SH = {}
    ken = d["const"]["ken"]
    for gd in d["gardens"]:
        ...
        if not sc: continue
        for sg in gd.get("singles", []):
            if (sg.get("layer") or "") not in (cr.get("roles") or []): continue
            r = single_crown(d, sg)
            if r is None:
                note.append("%s の樹冠が引けない(部材が目録に無い)— 退避を測れていない" % sg["name"])
                continue
            if sc not in SH: SH[sc] = avoid_shapes(d, g, sc)
            nm = None
            for k in range(24):
                a = 2.0 * math.pi * k / 24.0
                q = (sg["uv"][0] + math.cos(a) * r, sg["uv"][1] + math.sin(a) * r)
                nm = shape_hit(q, SH[sc]) or nm
            if nm:
                note.append("%s(%s・樹冠 %.2f m)の樹冠の外周が退避に掛かる — %s"
                            % (sg["name"], gd["name"], r * 2.0 * ken, nm))
    return bad, note


def single_crown(d, sg):
    """一本立ちの**据えたあとの樹冠の半径[間]**。`crown` があればそれ、無ければ部材と `scaleRule` から。"""
    ken = d["const"]["ken"]
    ...
    best = None
    for pt in d["planting"]["parts"].get(key[0], []):
        ...
        for rt in d.get("routes", []):
            ...
            if n * ct > 0.02:
                note.append("%s が 動線『%s』の通行帯(幅 %.2f m)と %.2f 坪 重なる(面の %.0f%%)"
                            % (gd["name"], rt["name"], rt["w"], n * ct, n * 100.0 / max(1, len(cells))))
    return bad, note
...
_OVL_OK = [
    ...
    ("玉垣:南面", "前庭の植込みの島"), ("玉垣:北面", "前庭の植込みの島"),
    ("玉垣:斜", "前庭の植込みの島"), ("玉垣:西面", "前庭の植込みの島"),   # 玉垣は島の輪郭に立つ
    ("踏石:仁王門の敷居前", "門:坂下の門(仁王門)"),                        # 踏石は敷居の前に接して敷く
    ("踏石:参道の階の上端", "石段:参道の階(前庭へ)"),
    ("踏石:参道の階の上端", "御成の供待"),                                 # 階の頭と供待は前庭の北縁を分け合う
]


def _ovl_shapes(d, g):
    """総当たりに載せる面 — 棟・門・庭/区・帯4・石段・塊。(名, 判定関数, bbox) の列。"""
    ken = d["const"]["ken"]
    out = []
    ...
    def corr_item(nm, pts, hw):
        ...
        out.append((nm, f, (min(us) - hw, min(vs) - hw, max(us) + hw, max(vs) + hw)))
    ...
    for k in d["kaidans"]:
        pts = [tuple(q) for q in (k.get("pts") or [k["a"], k["b"]])]
        corr_item("石段:" + k["name"], pts, (k.get("wKen") or k["w"] / ken) / 2.0)
    for holder in d["gardens"] + d["slopeBands"] + [d["planting"]]:
        for c in holder.get("clusters", []) + holder.get("viewClusters", []):
    ...
    for gd in d["gardens"]:
        for sb in shrub_segments(gd):
            poly_item("低木の帯:%s%s" % (gd["name"].split("(")[0], sb.get("name") or ""),
                      shrub_rect(sb))
        for nm, a, b, fence, _ln in tamagaki_edges(gd):
            if not fence: continue
            corr_item("玉垣:" + nm, [a, b], gd["tamagaki"]["postDiaM"] / 2.0 / ken)
    for nm, u0, v0, u1, v1 in fumiishi_rects(d):
...
def prop_rects(d):
    """面を持つ点景(`plan {du,dv}` と `yaw` を持つもの)の輪郭。(名, 4点[uv]) の列。
    ...
    for p in d.get("props", []):
        ...
        for i, (u, v) in enumerate(p["uv"]):
            ...
            out.append(("%s 其%d" % (p["name"], i + 1),
                        [(u + x * ca - y * sa, v + x * sa + y * ca) for x, y in q]))
    return out


def rect_overlap_check(d, g, minTsubo=0.05):
...
def tachiki_table(d, g):
    ...
    for gd in src:
        ...
        ar = ("%s 坪【算出】" % format(int(round(poly_area(P) * ken * ken / TSUBO)), ",")) if P else "前庭の中"
        first = True
        for c in gd.get("clusters", []):
            ...
            if c.get("overhangFrom"):
                oh = overhang_range(d)
                rl += ("(道への張り出し %.1f〜%.1f m【従属】)" % oh) if oh \
                    else "(道への張り出しは**部材が目録に無く測れない**)"
            pt = ""
            if gd.get("hBand"):
                seg = []
                for kind, part, n, h, cr in cluster_parts(d, gd, c):
                    ...
                    if cr and c.get("spacing"):
                        gp = c["spacing"] * ken - (cr[0] + cr[1]) / 2.0
                        ov = "／ 芯々−樹冠 %+.1f m" % gp
            ...
            rows.append("<tr><td>%s</td><td>%s</td><td>東縁の低木の帯 %s</td>"
                        ...
                        % (_gname(gd) if first else "", ar if first else "", sb.get("name", ""),
                           ...
                           sb["veg"], "板塀が無い区間(崖の肩)を低木で塞ぐ。"
                                      "島が接する区間は島が引き取る"))
            first = False
        tgs = tamagaki_stats(gd)
        if tgs:
            tg = gd["tamagaki"]
            eg = tamagaki_edges(gd)
            rows.append("<tr><td>%s</td><td>%s</td><td>玉垣(腰高の木柵)</td>"
                        "<td class='note'>立てる辺 %s ／ ⛔ %s</td><td>柱 %d 本</td>"
                        "<td>%g 間</td><td>高 %.2f m ／ 柱 φ%.3f ／ 貫 %d 段 ／ 立子 φ%.3f を %.2f m 間隔</td>"
                        "<td class='note'>延長 %.2f 間【算出】%s</td></tr>"
                        % (_gname(gd) if first else "", ar if first else "",
                           ...
                           (" ／ %s 幅 %.2f m を (%g, %g) に開ける"
                            % (tg["kido"]["name"], tg["kido"]["wM"],
                               tg["kido"]["uv"][0], tg["kido"]["uv"][1])) if tg.get("kido") else ""))
            first = False
        if gd.get("shrubs"):
            sh = gd["shrubs"]
            n = len(island_shrubs(gd, ken))
            rows.append("<tr><td>%s</td><td>%s</td><td>島の照葉低木</td>"
                        "<td class='note'>%s</td><td>%d 本【算出】</td>"
                        "<td>%.2f m</td><td>%s ／ 丈 %s m</td><td class='note'>%s</td></tr>"
                        % (_gname(gd) if first else "", ar if first else "", sh["insetRule"],
                           n, sh["spacingM"], sh["veg"], _rng(sh["hM"]),
                           "⛔ 本数は設計値ではない — 面積と芯々からの従属値"))
            first = False
        if gd.get("shitakusa") and gd.get("shrubs"):
            rows.append("<tr><td></td><td></td><td>島の下草</td><td class='note'>—</td><td>—</td>"
...
def budget_table(d, g):
    ...
    rows = ""
    tot = 0
    for key, n, avg, det in plant_budget(d, g):
        pend = any(pt.get("pending") for pt in d["planting"]["parts"][key])
        t = None if avg is None else n * avg
        if t: tot += t
        rows += ("<tr><td>%s</td><td>%s 本</td><td>%s</td><td>%s</td><td class='note'>%s</td></tr>"
                 % (key, format(n, ","),
                    ("—(**新造待ち**)" if pend else "—(**目録に無い**)")
                    if avg is None else "%s 三角/本" % format(int(avg), ","),
                    "—" if t is None else "%s 三角" % format(int(t), ","),
                    "Terrain の Detail Mesh" if det else "GameObject"))
    rows += ("<tr><td><b>計(測れた分)</b></td><td></td><td></td><td><b>%s 三角</b></td>"
             "<td class='note'>⚠ 落葉高木3種は目録に無く**この計に入っていない**</td></tr>"
             % format(int(tot), ","))
    return ('<div class="tw"><table><thead><tr><th>層</th><th>本数</th><th>部材の三角数</th>'
            "<th>小計</th><th class='note'>据え方</th></tr></thead><tbody>" + rows
            + "</tbody></table></div>")
...
def reviews_table(d):
    ...
    for key, nm in (("kosho", "考証方"), ("kenzu", "検図方"), ("niwashi", "庭方")):
        ...
        if not r: continue
        it = [q for q in r.get("items", []) if q.get("status") != "closed"]
        if it:
            body = "<b>未解決 %d 件</b>: " % len(it) + "・".join(
                "%s%s %s%s" % (q.get("sev", ""), q.get("n", ""), q.get("title", ""),
                               "(判断待ち)" if q.get("status") == "pending" else "")
                for q in it)
        else:
            body = "<b>すべて閉じている</b>" if r.get("items") else "件ごとの記録は未記入(要約のみ)"
        rows.append("<tr><td>%s</td><td>%s</td><td>%s</td><td class='note'>%s<br>%s</td></tr>"
                    % (nm, r.get("at", "—"),
    ...
    return ('<div class="tw"><table><thead><tr><th>検分役</th><th>日付</th><th>判定</th>'
            "<th class='note'>未解決の件 ／ 要約</th></tr></thead><tbody>"
            + "".join(rows) + "</tbody></table></div>")
...
def checks_table(rows):
    ...
    for nm, bad, note in rows:
        tr.append("<tr><td>%s</td><td>%d</td><td>%d</td><td class='note'>%s</td></tr>"
                  % (nm, len(bad), len(note),
                     "<br>".join(html.escape(q) for q in (bad + note)[:8]) or "—"))
...
def main():
    ...
    fig(h, keidai_svg(d, 9, 35, -13, 15, "%s 附図　前庭 平面" % KAN[n[0] - 1]),
        cap="<b>附図 前庭 平面。</b>前庭に囲い" + _zentei_kakoi(d) + "・坂下の門・茶店の縁台4・"
            "<b>玉垣で囲う植込みの島</b>【S 名所図会】・<b>御成の供待の空地</b>(破線)・"
            "参道の取り合いが集まる面。<b>表参は門口の芯を通って男坂の足へ真西から取り付く</b>"
    ...
    h.append('<p class="cap"><b>社叢が境内の実体である。</b>帯1〜3は法肩からの下りの割合で切り、'
             ...
             '⭐ <b>2026-09-06 に受入検査を不変条件へ置き換えた</b>(庭方 B-1)— 庭方が自らの実測を撤回し、'
             '<b>当図の定義(東〜北東は帯4の西縁を法尻)を正典とした</b>。'
             '⛔ <b>面積も本数も合否には使わない</b> — 見るのは上の表の三条だけで、'
             '<b>①面の総和が社地に閉じるか ②下るほど広いか ③帯が破片に砕けていないか</b>。'
             '⚠ ③は 0.5 間の格子で実地形を切る以上、縁に破片が必ず出るので<b>現況では成り立たない</b> — '
             '⛔ 閾値も定義も動かさず、「未解決」の節へ立てて庭方の判断を待つ。')
    ...
    h.append('<p class="cap"><b>建物は樹林の中の明地に建つ</b>【S】。'
             ...
             '⭐ <b>前庭は 2026-09-06 に庭方が設計した</b> — 高木は<b>榎一本</b>に減らし、'
             '松は<b>前庭の外(帯4)へ出して門被り</b>にした。<b>玉垣で囲う植込みの島</b>【S 名所図会】を'
             '東寄りに据え、縁台4基は<b>島の玉垣の前と榎の樹冠の下</b>へ寄せてある(⛔ 等間隔に並べない)。'
             '⛔ <b>御成の駕籠寄せは建てない</b> — 北縁に<b>供待の空地</b>を取るだけにする。'
             '⚠ <b>井戸屋形</b>は建物なので<b>ユーザーの裁定待ち</b>で当図にはまだ無い(「未解決」の節)。')
    ...
    h.append('<p class="cap">⚠ <b>常緑低木は在庫0件</b>(在庫方 2026-09-06)で、部材名は仮である — '
             '<b>社叢の下層・林縁・前庭の東縁・平場の縁の下層のすべてがこれ待ち</b>。'
             '⚠ <b>落葉高木3種(欅・椋・榎)は <code>docs/asset-index.tsv</code> に載っていない</b>ので、'
             '樹冠も三角数も<b>引けない</b> — 目録の再生成が要る。'
             '⛔ 低木・下草は GameObject にせず <b>Terrain の Detail Mesh</b> に載せる。')
    h.append("<h3>〔記録〕</h3>")
    ...
    h.append('<p class="cap"><b>参道は公道である。</b>植えられるのは<b>社地の側(西)だけ</b>で、'
             ...
             '<b>植込みそのものを囲う腰高の玉垣は社地の内(前庭)なら置ける</b>【S 名所図会】。'
             '⛔ 前庭の外周を柵で回さない。'
