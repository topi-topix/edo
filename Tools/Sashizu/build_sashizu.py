...
class Checks(object):
    ...
    def c04(self):
        ...
        for kind, b in M.buildings():
            ...
            worst = max(worst, mx)
            rows.append((b.label, b.name, min(ds), max(ds), pc))
            if pc < 100.0:
                bad.append("%s %.0f%%(最大 %.2f)" % (b.name, pc, mx))
        self.rows_c04 = rows
        self.add("C04", "面と地形の差 — 棟・附属屋の下で |設計面 − 自然地形| ≤ 0.5m(規則3)",
...
def build(est, deep=False, out=None):
    ...
    if getattr(C, "rows_c04", None):
        h.append(tbl(["棟", "欄", "Δ 最小", "Δ 最大", "|Δ|≤0.5"],
                     [[a, b, "%.2f" % lo, "%.2f" % hi, "%.0f%%" % pc] for a, b, lo, hi, pc in C.rows_c04]))
