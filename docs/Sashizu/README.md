...
2b. **⭐ 文章は指図 json に置かない — `<屋敷>_notes/<章>.json` へ置く**(2026-09-21 施主指示・土井で実施)。
   ⚠ 決めごと 2 を守っていても、**直しを重ねると `_` の注記として文章が json へ流れ込む**。
   2026-09-21 に測ったら大きい 4 邸はどれも **7 割が文章**で、値を一つ直すのに 700KB 読む羽目になっていた
   (松江松平 824KB / 山王 755KB / 土井 692KB)。⇒ 土井 **692KB → 227KB(33%)**。
   ```bash
   python3 Tools/Sashizu/prose_split.py <邸> --check     # 測るだけ
   python3 Tools/Sashizu/prose_split.py <邸> --apply     # 切り出す
   python3 Tools/Sashizu/prose_split.py <邸> --restore   # 戻して原本と突き合わせる
   ```
   **どの章を開けばよいかは `<屋敷>_notes/README.md`**(自動生成の目次)。`<章>/<添字>/<欄>` の
   path がそのまま鍵なので、直したい欄の path で grep すれば意図と経緯が引ける。
   ⛔ **文章を json へ書き戻さない。**⛔ **値を notes へ書かない。**
   ⚠ 動かさない物が三つある — **数値を含む `_` の欄**(測った値。土井 `terraceWalls/*/_exposure` ほか 42 件)・
   **生成器が刷る `_`**(`program/<i>/_` の 6 件。`READ_BY_GENERATOR` に列挙)・**`_` で始まらない欄**
   (`why` / `note` は読み手が使う本文)。
   ⛔ **切り出したら図を焼いて前後で diff する。**2026-09-21、刷られている 6 件を「刷られていない」と
   誤判定した(markdown が `<b>` を挿すので html への部分一致では見つからない)。html の diff だけが正しい検め。
...
**検査を足したくなったとき(邸の作業中に出てくる要求)の流れ**:
1. まず問う — 建つ姿を変えるか(重なり・浮き・埋没・区画侵犯・勾配)。⛔ 書式・帳簿・統計は足さない(規則4)。
   部材の取り合い・境界は Unity で測る(規則2・19)ので、紙の検査にしない。
2. 建つ姿を変える物は **共通の検査として** `build_sashizu.py` の `Checks` に `cNN` を 1 本足す(足すだけで全邸の輪に入る)。
   欄の綴りが邸で違えば `Model`(読み手)に吸収させる。⛔ 邸の名前で分岐しない。
3. その邸だけの条(例: 崖下の帯の棟)は検査ではなく **json の欄**(条文と数値)へ写し、図はそれを刷る。合否は Unity と庭方の目。
4. `check_triage.json` の `common` に意図を 1 行足す。壊して鳴るかは `--deep` の共通の試しに 1 種足す。
⛔ **邸ごとの生成器を新しく書かない**(`build_<邸>_sashizu.py` は 2026-09-20 に廃した)。

⛔ **「回していない」は「合格」ではない。** だから既定の run でも、
前回の記録の日付・鳴らなかった probe・記録の鮮度を必ず 1 行で刷る。
生成器・設計値・考証の文・共通ライブラリのどれかが記録の後に変わっていれば
⚠ を出す(指紋は `sashizu_lib.sens_fingerprint`)。記録が無い束は ⛔ で
「**回っていない**(合格ではない)」と刷る。

**`--deep` を回すのは、破壊試験の記録を新しくしたいときだけ。**
設計値を直している最中の毎回の run では要らない — 記録が古いと刷られるので、
一連の直しが終わった区切りで 1 回回せばよい。

⭐ **なぜこうしたか**(実測 2026-09-20): 松江松平の生成器は 1 回 120〜178 分で、
そのうち約 80 分が破壊試験だった(`_garden_checks` 1 回 61 秒 × 81 probe)。
9/15〜9/19 の 41 回ぶんの待ちがそのまま役の実働になり、前日の実働 24.9 時間の
65% を占めていた。破壊試験は**検査の検査**で、設計値を 1 つ直すたびに要る種類の
検査ではない。

**どの検査が重いかは毎回刷る**(`sashizu_lib.chk` / `chk_report`)。
20 秒を超えた検査はその場で ⏱ を出す。輪を長くしている検査はここで見つける。
...
| 松平出羽守 上屋敷（出雲松江藩18万6千石・親藩国主） | 山王社北 | **三役 pass(第16次・2026-09-06)** → 西斜面の植生の直し(ブックマーク#1)と石組 17 石を実装中 | 実装中(Stage6まで・継続更新) | [matsudaira_dewa_sashizu.html](matsudaira_dewa_sashizu.html) | https://claude.ai/code/artifact/21e27f80-8a85-4ada-aac9-9baef15464ee(⚠ 2026-09-04 に URL が変わった — 旧 eaba651e… は別セッションの公開で上書きできなかった) |
| 土井大隅守 上屋敷（三河刈谷藩2万3千石・譜代雁間） | 山王社北 | **検図関門 ⭕ 3/3（2026-09-04・検図/考証/庭方 pass・奥庭を新設・図版41面）** | **未着手** | [doi_sashizu.html](doi_sashizu.html) | https://claude.ai/code/artifact/8a6e1da5-eb75-41df-af1b-7ce76586d2b9 |
| 山王権現社（日枝神社・社領600石【S】） | 永田馬場 星ノ山（読み未決） | 検図⛔・考証⛔（是正3巡済・4巡目未）・庭方⚠検め直し | 実装は透塀のみ是正済(EDO-0073・実行検証未) | [sanno_sashizu.html](sanno_sashizu.html) | https://claude.ai/code/artifact/d13f7f31-2003-406f-8ca4-4d49157fc491 （旧 b6b4e5cd… は 08-26 版のまま） |
