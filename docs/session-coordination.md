...
### ⛔ `ps` に pid が無いことを「そのセッションは終わった」の根拠にしない(EDO-0280)

各セッションは**自分以外の pid がプロセス表に見えない**環境で走っている。相手が元気に
動いていても `ps -p <pid>` は空を返すので、これを生死の判定に使うと**必ず**相手を
死んだと誤判定する。2026-09-20 に山王が「松江松平(pid 42704)がプロセス表に無い」と見て
unity を取り上げ、実際には据え直しの2巡目の最中だった松江松平は作業を折られたうえ、
取り上げられたことにも気づけないまま待ち行列で 38 分待った(EDO-0271)。

- **生死は claim の心拍だけ。** `status` に出ている claim は、出ている時点で全部生きている
  (TTL 45 分を過ぎた claim は load_all がその場で落とす)。
- **`steal` では生きている保持者から資源を取れない。** 門番が拒否して `wait` を案内する。
  本当に固まった相手は心拍も止まり、`IDLE_MIN`(unity は 20 分)を過ぎれば「放置」として
  正規の手順で引き取れる。**逃げ道は要らない。**
- **取り上げは相手の claim に記録が残り**、相手の `status` に
  「⚠ unity は <誰> が引き取った(<いつ>・<なぜ>)」と出る。黙って消えない。
...
- **生存判定は心拍だけ。** ⛔ pid を見てはならない — フックから呼ばれるスクリプトの親は
  ...
  ⛔ **`ps` も同じく使えない**(他セッションの pid はプロセス表に映らない・EDO-0280)。
  コードに pid の判定は一箇所も無い — 誤判定はいつも**人が `ps` を打って**起きる
